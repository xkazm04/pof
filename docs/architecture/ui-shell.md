...
| `src/components/layout-lab/CatalogMatrix.tsx` | Catalog-wide status matrix: entities (rows) × steps (columns) colored by derived Acceptance; per-entity `summarizeEntity` rollup + blocker flags; cells jump to that entity's step. Header hosts the batch-drain action; a draining row shows a left-accent + "draining…" badge |
| `src/components/layout-lab/MatrixBatchDrain.tsx` | Matrix header action — "drain all deferred gates in this catalog" (shown only when ≥1 entity is deferred). Serial progress, Cancel, and a flips summary (passed/failed/still-deferred/locked) with per-step fail reasons |
| `src/components/layout-lab/hooks/useBatchDrain.ts` | Serial batch-drain engine: iterates deferred entities ONE at a time (single-lease UE runner), retries a 409 lease once then records it locked, invalidates the shared cache per entity for live grid updates, and is cancellable (stops after the in-flight entity) |
| `src/components/layout-lab/batchDrainModel.ts` | Pure batch-drain model: `DrainOutcome` (ok/locked/error) + `foldEntityOutcome` accumulating the catalog-wide flips summary |
