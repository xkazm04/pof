...
- **GAP (external — UE scenario harness, not the lab):** headless `-game -PoFScenario` Move-injection drives **low/inconsistent displacement** (~10u typical, 65u once) with `anim_speed 0` + near-static droop — i.e. the injected Move (key OR action+value) isn't producing **sustained locomotion** in `-game`. Likely the camera/control-rotation is uninitialized headless so `HandleMove`'s camera-relative transform zeroes forward, and/or VerticalSlice damage/obstruction. Lives in the observation-spine `UScenarioController` C++ (`[[project_llm_ue_interface]]`), reconsider when that's revisited. The behavioral-assertion + capture path itself is proven; only the input→locomotion magnitude is gated.
