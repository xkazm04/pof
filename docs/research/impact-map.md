...
- **Already-has:** reuses `ue-launch` (launch/python/capture/markers) + `/api/verify/visual` (L4) + `parseScenarioVerdict` + the `harness-runs-db` persistence pattern. **Proven live 2026-06-19** (real UE 5.8: 439 KB frame + `RESULT=5.8.0`; gameplay scenario: 8 samples, loc 0→65, health 85→55). Headless-per-run; ad-hoc; in-app UI.
