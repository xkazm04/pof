...
- **✅ Physical removal DONE 2026-06-19:** deleted the `MCPUnreal` UE plugin (`<project>/Plugins/MCPUnreal/`) + the Go binary (`~/mcp-unreal-staging/`). Both recoverable (UE git history / upstream re-clone). No dangling refs (`PoF.uproject` didn't pin it; no `Source/` dep). UE-side deletion left unstaged for the concurrent session to commit. Only `PillarsOfFortuneBridge` (`:30040` moat) remains.
