...
> **Verification still pending a live run:** the plugin C++ compiles against stable Core automation APIs but hasn't been built/run here (no editor up, shared tree). The remaining real-world proof is: rebuild the editor, start it (bridge on `:30040`), and run a drain against a catalog with a real `VS*Test` (e.g. `items` → `VSItemsDefinitionsTest`).
