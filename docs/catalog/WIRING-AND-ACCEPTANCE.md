...
- **`/api/pipeline-artifacts/verify-static`** — runs a step's declared `staticChecks` (`cppSymbolExists` / `seedRowPresent`) against the real UE tree (`src/lib/catalog/acceptance/staticVerify.ts`). **One writer per status:** it skips (`summary.delegated`) every packaging step — `verify-packaging` owns those and folds their `staticChecks` into its own verdict — and it never lifts a row its content checker holds at `pending`/`fail` (the checker is re-run raw, no judge overlay; worst-of via `acceptance/combineVerdicts.ts`). A content `deferred`/`pass` leaves the static verdict standing, so L3-gated steps are untouched. Before 2026-09-24 each sweep wrote alone and the last writer won: static would pass a package with unrealized `/Game` declarations, and would erase a declared-gap `pending`.
