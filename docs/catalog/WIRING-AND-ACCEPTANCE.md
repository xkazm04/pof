...
- `catalog_entities` — `(catalog_id, entity_id)` → the whole `StoredCatalogEntity` as JSON + `source` (`user|one-shot|ingest`; an `ingest` row carries per-row `provenance` incl. a licence note — `src/lib/catalog/ingest/`). The durable record of a USER-created entity; `seededEntities()` returns the union of the code seeds and these rows (a persisted id can never shadow a code seed — the collision is reported by `entityCollisions`, never merged). Written through `POST /api/catalog-entities` only. Without it a one-shot draft lived in `localStorage` while its artifacts went to SQLite, so no server path could resolve it and every gate silently exempted it.
...
**A fix that cannot work is not offered (2026-09-04).** `StepSpec.produce(entity, direction)` takes the operator's direction, and **no registered body reads it**: measured over the 33 pipeline files, **344 `produce:` literals, 0 declaring a second parameter, 0 containing `Math.random`/`Date.now`**. So on an ALREADY-PRODUCED static step, "⚡ Produce fix" re-wrote byte-identical data — the verdict could not move, forever — while the banner previewed the derived corrective direction verbatim, as if it were about to be acted on. ~303 steps were offered that button.

- **`readsDirection(spec)`** (`@/lib/catalog/stepSpec`) answers "does this body consume the direction?" by an **arity probe of the body itself** (`produce.length >= 2`), with an explicit `StepSpec.readsDirection` override for the one case arity gets wrong (rest params, a defaulted parameter, `arguments`). The probe is the default deliberately: a flag on 344 specs is a claim ABOUT the body that rots out of step with it, while the probe reads the code.
- **`fixEffectOf`** (pure, in `ArchetypeStep`) classifies each step's fix: `reroll` (gallery — a corrective batch is real new content), `first-produce` (never produced — producing from nothing is a change whatever the body reads), `live-produce` (live CLI mode on a CLI-eligible archetype — a model authors the payload from the direction), or `no-op`. Only `no-op` loses the button, and it keeps `deferred`'s existing behaviour untouched.
- **The banner states the remedy**, never a bare absence: `noopFixSuggestion` says the body ignores the direction and is deterministic, names what WOULD change it (turn on live produce for `brief`/`graph`/`rules`; otherwise the produce body must be authored to read its `direction`), and still carries the derived direction — as an input to use elsewhere, not as an imminent dispatch.
- **Ratchet.** Spec-linter rule **(p)** (`pipeline-spec-linter.test.ts`) records the direction-blind count per archetype — brief 34 · rules 126 · gallery 47 · checklist 61 · manifest 35 · balance 16 · schema 14 · graph 6 · custom 5 = **344**, the whole fleet — shrink-only with a no-slack check, exactly like the `StepSpec.engine` ratchet. That is the honest starting line, not an exemption: wiring direction into the produce bodies is the follow-on work, and these numbers can only go down.

**Every artifact records its producer (2026-09-04).** `data._provenance` was designed, typed, documented and READ by the Evidence modal — and never written: measured read-only on the live DB, **805 of 817 rows carried `engine:'unknown'` and the other 12 carried no stamp at all (100% not recorded)**, because `stampPromptVersion` was the only writer and it defaults the engine to `unknown`. The two produce write paths now stamp what they can prove.

- **`POST /api/one-shot/step`** stamps `Claude` in `cli` mode with the model/effort it already resolved for the dispatch (`resolveDispatchModelChoice` → `claudeProvenance({model, effort})`, which now accepts an already-resolved choice so the stamp cannot name a different model than the one that ran; an UNPINNED dispatch omits the fields rather than inventing a pin), and `Code` in deterministic mode — a step body is an author, and the `Code` stamp is what makes a stored-but-never-sent prompt legible instead of implying a session.
- **`POST /api/pipeline-artifacts`** takes an optional `engine` from a CLIENT allow-list (`CLIENT_DECLARABLE_ENGINES = ['Code']`; anything else is a 400 naming the field). A client may only assert about itself: `Claude`/`Leonardo`/`Tripo`/`ElevenLabs` claim that a remote engine RAN, and accepting that from a browser would be a fabricated-provenance hole of the same shape as a fabricated `pass`. An unverifiable engine smuggled inside `data._provenance` is downgraded to `unknown` — **except** when the server already recorded that exact engine for that row, so the lab's write-through re-POST of a live one-shot output keeps its true `Claude` stamp. `postArtifact` declares `Code` only when the payload does not already name a producer.
- **Provably verdict-inert.** `_provenance` is in `NON_CONTENT_KEYS`, so `stepContentHash` ignores it: stamping or changing an engine leaves the hash byte-identical and can neither mark a verdict stale nor trip the drift banner (pinned by `src/__tests__/catalog/provenanceStamp.test.ts`).
- **Absence reads as absence.** The Evidence modal renders an unstamped or `unknown` row through `describeProducer` as 'producer: not recorded', never as a producer named unknown. Existing rows are **not** backfilled — they genuinely are unknown.
...
**Headless-operability gate (/status).** A `verified` grade claims professional-grade, machine-reproducible proof — so a cell cannot grade `verified` unless its step is proven **headless-operable via pof-mcp** (drivable/verifiable without a human at the editor). A coverage walker, `scripts/headless-coverage.mjs` (regen: `node scripts/headless-coverage.mjs` with the dev server running), enumerates every registered step and records `{catalogId, step, operable}` into `src/lib/status/headless-coverage.json`. `statusModel.gateHeadless(cell, catalogId, step, lookup=getHeadlessFact)` (pure, lookup injectable) demotes a would-be-`verified` cell to `trusted` — the honest ceiling for a claim the machine can't reproduce — when the step has no coverage entry OR is `operable:false`, prefixing the reason `not headless-operable via pof-mcp`; all other grades pass through unchanged. `getHeadlessFact(catalogId, step)` exposes a single fact. `buildSwimlane` applies the gate to every cell after `deriveCell` (via an injectable `headless` parameter defaulting to the json-backed lookup), so `readyPct` reflects the gated grades. `deriveCell` itself is untouched — the gate is a `/status`-only overlay, like the judge bridge.

**Readiness ladder — ONE scale on the map (2026-07-29).** /status used to paint **two** competing colour languages on every cell: the background encoded `CellGrade` (credibility) and a 3px left stripe encoded the acceptance tier `L0–L4` (which KIND of check was reached). They fought each other — `TIER_VAR.L4` and `GRADE_VAR.verified` were the same green token, so green meant "*declares* a visual gate" on the stripe and "a gate actually *passed*" in the fill; the stripe read `bestPassTier ?? tier`, whose fallback takes the max tier of ANY artifact **regardless of status**, so a merely-DECLARED, deferred L4 painted a green stripe on an amber cell (rewarding ambition over achievement); and `--lab-ink` === `--lab-accent` on the Blueprint floor, so `L1`/`L2` were literally indistinguishable. 8 grades × 5 tiers = 40 combinations the reader had to compose mentally.

`src/lib/status/readiness.ts` (pure) collapses both into ONE ordered ladder of production readiness, `R0`–`R5`: **R0 NOT WIRED** (no artifact) · **R1 HOLLOW** (`unpowered`/`pending` — a pass on placeholder data, or nothing real behind it) · **R2 DRAFTED** (`ungated`) · **R3 REVIEWED** (`trusted`) · **R4 PROVEN** (`verified` — a real L3/L4 gate passed headless-operable, or a strict judge at the shippable bar) · **R5 SHIPPED** (R4 **and** `realization.ue === 'proven'`). Two states are deliberately **not rungs**: `waiting` (a `deferred` cell — the gate is declared but was never run) renders **hatched with a dashed border at the rung it is aiming for**, so a declared gate can never read as progress; `blocked` (`attention`) renders terminal red with a `✕`, carrying a would-be level only so it sorts and filters coherently.

The ramp is monotone and **green stays reserved for R4+** (a `readiness.test.ts` invariant asserts exactly that, plus that `RAMP` covers the ladder in order — the guard that keeps the scale honest under future edits). Colour never carries a rung alone: every cell renders its `R…` code (`data-testid="readiness-code"`, replacing the old `tier-code`) plus the state glyph. **The acceptance tier is not lost** — it survives as *evidence-class* metadata in the cell tooltip/aria-label and in the Evidence modal; it is simply no longer a second rating. Lane percentages were **renamed, not silently redefined** (`verifiedPct`→`readyPct` R4+, `credibleGePct`→`crediblePct` R3+, `wiredPct`→`startedPct` R1+, plus `blockedCount`), so no caller can keep reading the old meaning. **No grading logic moved**: `deriveCell`/`CellGrade` still own headless gating, rubric filtering and unpowered detection, and `readinessOf` is a pure projection over the already-derived cell — consolidating the display provably cannot move a verdict. The one deliberate exception is R5, which promotes `realization-facts.json` from decoration to a readiness **display** input (acceptance, `gradeArtifact` and the checkers never see it; `preview/realization.ts` says so).

**Craft axis — the PARALLEL A-scale (2026-07-30).** The R-ladder measures *automation honesty*; it says nothing about whether the output would survive review at a AAA studio. `src/lib/status/craft.ts` (pure, mirrors `readiness.ts`) adds the orthogonal **A-axis**: **A1 HOBBY** · **A2 INDIE** · **A3 AA** · **A4 AAA-PARITY**, with **A0 UNGAUGED** as projected absence (never stored). Two states: `^` **at-ceiling** — the step reached the recorded maximum its *medium* permits (`src/lib/craft/craft-ceilings.json`, per deliverable class with reason + class: `3d-mesh`/`animation` **A2 permanent** — the market bet that generative 3D/anim never reaches top tier — `2d-art` A3 arguable, text/code/audio/vfx A4 uncapped) and renders as **achievement**, never amber shame; `~` **stale** — the artifact changed after it was gauged (anchor: `artifact_updated_at` stamped at the verdict write seam), so the level grades content the step no longer holds. A verdict under an older **lens version** projects as A0 — lenses (`src/lib/craft/lenses/*.md`, ten research-grounded rubrics with named benchmark anchors and cited criteria; versions pinned in `lens-versions.ts`, mapping in `lens-map.ts`, format enforced by `lens-files.test.ts`) only change via a version bump that *visibly* invalidates dependent gauges. Storage is a **separate table** `craft_verdicts` (`src/lib/craft/craft-verdicts-db.ts`, route `/api/craft-verdicts` — findings are criterion-named and routing-classed `content`/`capability`/`ux`; the per-catalog `production-process` scorecard lives under the `__catalog__`/`__process__` sentinel row), deliberately apart from `judge_verdicts` so a craft gauge can never enter `statusModel`'s judge-elevation. **Provably display-only**: `craftDisplayOnly.test.ts` fails if any grading module (`acceptance/`, `statusModel`, `readiness`) imports a craft module. On the map, each cell carries an A-chip beside its R-code (`StatusCell`, `data-testid="craft-code"`; attached post-hoc by `PipelinesView` via `craftCell.ts`'s `craftForCell` — a cell shows its WORST entity gauge, and a failed craft fetch hides the chips entirely rather than painting A0 over a load error) and the chip rows gain a **distance-to-roof rollup** (Σ ceiling − level per lens; ungauged/stale count their full ceiling distance, so the number only shrinks through real gauges). The loop that populates it is the **craft-loop** skill (`.claude/skills/craft-loop/SKILL.md`): KNOW (lens library) → AUDIT (read-and-score fleet + spot-check) → ROUTE (`content` findings → green-loop worklists, `ux` → the ideas backlog, `capability` → the ceilings file) → WRAP. Perfect tier for a step = **R5 × A-at-its-ceiling**; design spec at `docs/superpowers/specs/2026-07-30-craft-loop-design.md`.

**Operator-paired walk — personal-loop (2026-09-15).** The autonomous loops above pick their own batches; the **personal-loop** skill (`.claude/skills/personal-loop/SKILL.md`) walks the same map *with* the operator, one cell at a time, in a **frozen column-major order** (every pipeline's step 1 in `sortLanes` order, then every step 2, …). The order is snapshotted because `sortLanes` re-sorts by readiness and would otherwise reshuffle the walk as cells lift. Per item: baseline → pilot-entity best output through the product's own produce path → a mandatory operator stop (feedback on UI / prompt / functionality) → fan-out to the catalog's entities → recertify on both axes. `src/lib/status/personalLoop.ts` (pure) owns the order and the movement rules: a newly waiting/blocked cell is never progress, and a stale or absent A reading after work reads `unmeasured`, never a pass. `scripts/personal-loop/{order,recertify}.ts` build lanes with the map's own `buildSwimlane`/`readinessOf`/`craftForCell` and the DB modules' row mappers. Memory (walk status, settled topics, baselines) lives in the Obsidian vault's `Personal/` folder, not in the repo. Design spec: `docs/superpowers/specs/2026-09-15-personal-loop-design.md`.

**Gauge history + metering (2026-08-18).** A re-gauge used to DESTROY the evidence that a cell moved:
`craft_verdicts` holds exactly one row per `(catalog, entity, step)`, so raising a cell A1→A3 left no
trace, which is the same gap `judge_verdict_history` was built to close on the R-axis.
`craft-verdicts-db.ts` now also appends an **additive second table** `craft_verdict_history` (created
via the migration-latch pattern; `craft_verdicts` still holds ONE row per cell, so the chip is
provably unchanged), both writes in a single transaction against one `judged_at`, bounded to
`CRAFT_HISTORY_LIMIT` (20) per cell and pruned inside that transaction. Verdicts already on record are
seeded as each trend's first point — leaving them alone would make the first re-gauge read as the
trend's origin, losing exactly the evidence the log exists for. `GET /api/craft-verdicts` attaches each
cell's `movement` from ONE extra query so `craftForCell` can state "moved A1 → A3" without a per-cell
round-trip; `?history=1&catalogId&entityId&step` returns a single cell's full trend. Projection lives
in the pure `craftCell.ts` (`buildCraftTrend` / `craftMovementOf` / `craftMovementLabel`); a
once-gauged cell carries no `movement` key rather than a fabricated "unchanged".
Craft writes are also **metered** into `cli_spend` under module `craft` and task types
`craft-gauge` / `craft-process`, so the A-axis stops being the only quality axis whose spend appears
in no ledger. Cost is **optional** on the POST — the craft-loop skill is the only thing writing
verdicts today and must keep working — and an unreported cost records `costKnown:false` with a
labelled marker, never a `$0.00` that reads as free. Metering runs OUTSIDE the verdict transaction so
a ledger failure can never lose a committed gauge. All of this is additive and read by nothing in the
grading path: `craftDisplayOnly.test.ts` still fails if any grading module imports a craft module, and
new assertions pin that neither the history table nor the trend helpers are referenced from
`acceptance/`, `statusModel.ts` or `readiness.ts`.
...
- **`/api/pipeline-artifacts/verify-static`** — runs a step's declared `staticChecks` (`cppSymbolExists` / `seedRowPresent`) against the real UE tree (`src/lib/catalog/acceptance/staticVerify.ts`). **One writer per status:** it skips (`summary.delegated`) every packaging step — `verify-packaging` owns those and folds their `staticChecks` into its own verdict — and it never lifts a row its content checker holds back at a DATA tier — `pending`/`fail`, or `deferred` at L0-L2 such as an unresolved link (`holdsBackAtDataTier`; the checker is re-run raw, no judge overlay; worst-of via `acceptance/combineVerdicts.ts`). A content deferral at L3/L4 belongs to a runtime gate, so drained passes survive the sweep. Before 2026-09-24 each sweep wrote alone and the last writer won: static would pass a package with unrealized `/Game` declarations, and would erase a declared-gap `pending`.
- **`/api/pipeline-artifacts/verify-packaging`** — the **packaging truth engine** (`src/lib/catalog/acceptance/packagingVerify.ts` + `src/lib/catalog/packaging/`): for every step matched by `isPackagingStep` (the `packaging: true` StepSpec flag, or the canonical `"UE Packaging"` label — so all pipelines are covered without per-pipeline edits), it **rebuilds the row's package from its SIBLING artifacts** — referenced files (incl. `/api/visual-gen/asset/…` URLs mapped to their `generated/triposr/` disk homes) hashed in place, embedded data-URL art materialized as real files — into `generated/packages/<catalogId>/<entityId>/` with a `manifest.json` (`files[]` bytes+sha1, honest `missing[]`, `ueDeclarations[]`). The verdict derives from the manifest: all staged files **and** all `/Game/...` declarations disk-realized (`Content/<path>.uasset|.umap` under the resolved UE root — Tier 2) → **L2 pass**; anything missing → **deferred** with the paths/declarations as reason; declarations unchecked (no UE root) → files-only verdict, saying so. A clean Produce can defer here, never fail; only the folded-in static half can fail (`summary.failed`). Both sweeps import `registry.generated` themselves — without it a cold server grades nothing and reports success. `GET` = dry-run preview, `POST` = apply. An L3 rung exists for the declarations too: `scripts/ue/verify_package_declarations.py` (headless pythonscript commandlet, `does_asset_exist` markers, manifest via `POF_PKG_MANIFEST`) — live-validated to agree with the disk check. Spec/history: `docs/research/packaging-truth-engine-spec.md`. **Coverage is a two-state rule:** a pipeline either owns a packaging step or declares `packagingExempt: '<reason>'` on its `registerCatalogPipeline` call (`character-pipeline`, `player-movement`) — the drain returns those as `summary.exempt` so a catalog it cannot touch reads as an explicit exemption, not as nothing; `src/__tests__/catalog/packaging-coverage.test.ts` rejects a pipeline with neither.

**A third disk-truth pass re-binds generated 2D art** — `/api/pipeline-artifacts/bind-icons` (`src/lib/catalog/acceptance/bindGeneratedIcons.ts` + `bindIconsAll.ts`). `generated/icons/` holds gated images named for the exact `(catalogId, step)` they were generated for, and the lab already renders them (`ArchetypeStep` → `/api/visual-gen/icons` → `imageGalleryCandidates`) — but the PERSISTED artifact never learned about it: a produce stub seeds only deterministic swatch candidates (`gallerySeed`), so `gradeGallerySelection` reports `deferred` ("not a generated asset") for a step whose art demonstrably exists on disk, and **every re-produce re-opened that hole** (which is what kept sending the map's media cells back to R4-waiting after each pipeline campaign). This pass binds the library image onto the artifact's SELECTED candidate and re-grades through the normal `gradeArtifact`. It cannot manufacture a pass: the file must exist and match the artifact's own identity by the generator's own filename rule (a step can never borrow a sibling's art), a candidate that already carries a real asset is left alone, an artifact with **no** generation history is skipped (binding art onto a batch that never ran would invent one), and the bind is disclosed in the artifact data under `iconBinding`. `GET` = dry-run preview, `POST` = apply; idempotent.

**It has a lab affordance — stop re-running it by hand.** `MatrixBindIcons` (`src/components/layout-lab/MatrixBindIcons.tsx`) sits beside the batch-drain control in the catalog matrix header (mounted from `MatrixBatchDrain`, its sibling disk-truth pass, and rendered even when there is nothing left to drain — which is exactly when a campaign needs it). **Preview** issues the `GET` dry run, **Bind** the `POST`; both render the route's OWN counters — bound / verdict deltas (`deferred → pass`) / skipped with the checker's own reasons / how many artifacts the library holds no art for at all — and which SCOPE served each bind (`entity` art vs the per-step icon standing in). What the pass needs (files in `generated/icons/`) is disclosed *before* the click, and **`0 bound` is a first-class line naming its top reason** (an empty library says so) rather than a silent no-op or a bare "done". The wording is derived by the pure `describeBindOutcome`, so it is testable without a fetch. Deliberately NOT an automatic post-campaign hook — that is a behaviour change with spend implications.

**The icon library is keyed on the ARTIFACT identity `(catalog, entity, step)`, never a display slug** (`src/lib/visual-gen/generated-icons.ts`). A filename carries an optional entity dimension and both shapes are first-class: `catalog__entity__step.*` parses to `{catalogId, entityId, step}` (`scope: 'entity'`, the literal `__` separators read from the RAW basename *before* normalization collapses them), anything else is a per-step file identified by its flat slug alone and resolves byte-identically to how it always has — a legacy name is never split on a guess. Build names with `iconFileBase(catalogId, step[, entityId])`, match with `iconSlug(catalogId, step[, entityId])`, and resolve with `iconsFor` / `resolveIconFor`, which apply **one precedence at every read site** (the icons listing route, `bind-icons`/`bindIconsAll`, `imageGalleryCandidates`): an entity-specific icon wins for that entity, the per-step icon is the fallback, and the result DECLARES the `scope` that served it — a step icon standing in for an entity is a stated substitution, and a bind row records which scope bound it. `iconsForStep` deliberately returns step-scoped art only, so one entity's art can never answer for the whole catalog. This is why art generated for a single entity used to be *structurally unaddressable* (`power-icon.mjs` invented `catalog__entity__hero` names no consumer could match); the orphans that survive it are preserved, declared, under `generated/icons/_unaddressable/` — an unaddressable asset is a declared state, never a silently skipped file.

**And an L3 drain against the LIVE editor** — `/api/pipeline-artifacts/drain-python` (`src/lib/catalog/acceptance/pythonDrain.ts`), the missing production consumer for `pythonStepCheckers.ts`. The `player-movement` steps each persist a dispatch descriptor (`data.python = {module, function}`) and their checkers read the module's structured return (`{created,skipped,failed}` / `{ok,issues}`) back out of the artifact — both halves existed (modules on disk in `Content/Python/player_movement/`, checkers unit-tested) but nothing ever called the bridge and wrote the result back, so all ten steps sat permanently `deferred` ("awaiting /pof/python/run on the editor") = R4-WAITING on the map. This runner calls `runPython` per step and re-grades with the step's own checker. Honesty rules: a step with no descriptor is skipped (never invents a dispatch); the verdict is the module's, re-graded by the normal checker, never the drain's own; a **transport failure writes NOTHING** — "we could not ask" is not "the step failed", so the artifact stays `deferred` and the error is reported in the summary; and steps run in the pipeline's declared ORDER with a chain stopping after a condemned step, since `retarget` failing merely because `import_clips` never ran is a consequence, not an independent finding. **Prerequisite is the opposite of `/drain`'s:** this needs the editor RUNNING (bridge on `:30040`, from the `PillarsOfFortuneBridge` editor plugin), where `/drain` spawns its own headless editor and needs it CLOSED. `GET` previews the dispatch list without touching the bridge.
