...
- **Node kinds before rates.** Every node in an economy is named as exactly one of **source / drain / converter / trader / pool** (`EconomyNodeKind`; `nodeKindOf()` in `src/lib/economy/node-audit.ts` — `faucet`/`sink` are the legacy spellings of source/drain). A **converter** (vendor sale: items in, gold out) declares BOTH legs — its `input` leg is required, and the simulation consumes it, so an empty pool means no conversion. A converter recorded on one side only is an unfunded faucet: inflation with no author. **The band is not the verdict:** `runSimulation` returns `audit` + `balance`, and an unclassified node, an unfunded converter, an undeclared source (a clamp that mints) or a declared frequency the structure cannot supply makes the verdict `unclassified` / `unspecified` / `not-measured` — never `pass`. The feedback loops are declared in `DECLARED_LOOPS` with polarity, latency and a gain that reads `uncharacterised` until somebody estimates it.
