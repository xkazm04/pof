...
- `produce(entity, direction?)` → `{ data, ueAssets? }` — the produced payload. Asset names use UE prefixes (`T_`/`SM_`/`MI_`/`A_`/`NS_`/`GE_`/`DT_`, see canon `proj-naming`). Use `()` not `(e)` when the entity is unused (eslint).
  ...
  - **Live CLI produce (opt-in, off by default):** every CLI-eligible step (`brief`/`graph`/`rules`) renders a **Stub ⇄ Live CLI switch next to its Produce button** — the mode is legible at the point of produce, and the button reads `⚡ Produce … · LIVE` when a click will spend model budget. In **Live** mode the dispatch goes through the real `POST /api/one-shot/step` (`mode: 'cli'`) with the typed direction and adopts the artifact the SERVER persisted; a failure surfaces as the inline reason + "Retry with same prompt" (Rule 4). The choice persists per browser (`localStorage['pof-lab-live-produce'] === '1'`) and is broadcast to every mounted Produce panel; the dispatch path re-reads it at click time, so it can never act on a stale render copy. Default (stub) mode is unchanged and writes synchronously, which is what keeps the Rule 5 walker green. See `src/components/layout-lab/labProduceMode.ts` (`useLiveProduceMode` / `setLiveProduceEnabled`).
- `accept` — a **derived** `Checker` from `src/lib/catalog/acceptance/`:
  - **L0 (data):** `minLength`, `fieldsPopulated`, `withinPercent` (accepts a dot-path), `minCount`, `entriesHaveFields` (`dataCheckers.ts`), plus `keysNumeric` for the SHAPE of an object's values — each key one number or exactly `{minimum, maximum}`, optional canonical key names (`experience`) — with `statNumber` as the ONE reader a consumer uses (never guess a producer's nesting; /diablo W08 found 3 `damage` shapes in 5 Stat Blocks). A declared gap (`"not in the reference"`) is a gap in any wrapper — a bare string, `{ value: … }`, or an object whose own property is the marker with no number beside it (`isDeclaredGap`). A cast gate is `cooldownOrResourceGate` (`cadenceCheckers.ts`, /diablo W12, D4): a cooldown > 0, or no cooldown with an EXPLICIT `gatedBy: "resource"` beside a mana cost — the declaration is required so a forgotten cooldown still reads as missing. Prefer a CONTENT assertion over a bare count: `allOf(minCount('rows', …, 6), entriesHaveFields('rows', …, ['from','to','guard']))` proves the rows are actually filled in, not just numerous.
  ...
  - **L1 (human selection):** `selected` (`dataCheckers.ts`) — the gate for art/gallery steps. It grades the **selected candidate** resolved out of `data.genHistory`, not merely that an index exists: a real generated asset → `pass`; a deterministic swatch → `deferred` (L4) with a reason; an unresolved or index-mismatched selection → `fail`. Seed the artifact shape it reads with `gallerySeed(field, count)` (`acceptance/galleryArtifact.ts`) in the step's `produce`, and compose `selected(...)` **LAST** in an `allOf` so its deferral cannot mask a failing sibling check. Full rules: `WIRING-AND-ACCEPTANCE.md` §2.B.
  ...
  - **Content invariants (required for `balance`):** `acceptance/invariants.ts` — canon-parsed laws (`powerWithinTierTarget`, `priceRatioWithinBand`, `faucetSinkBalanced`, `requiredLevelBand`, `rarityAffixBudget`, `monsterRarityWithinBands`, `xpGrowthWithinBand`, `statusBalanceEnvelope`), arithmetic reconciles (`componentsSumTo`, `sumReconciles`, `arithmeticReconciles`, and `hitRateFromLimiter` in `cadenceCheckers.ts` — `hitDPS` over the limiter that BINDS, the longest of cooldown / castTime / manaCost÷manaRegenPerSec, never a quoted constant) and self-consistency laws (`budgetWithinCap`, `valueWithinDeclaredBand`, `descendingSeries`). Compose with `allOf(<shape check>, <invariant>)`. **Linter rule (k):** every `archetype: 'balance'` step MUST compose ≥1 — a shape-only balance step can never fail on a wrong number. If your step has no declared target/cap, WRITE one on the artifact and grade it (see `economySim.marginTargetPct`, `gameTier.sizeCapMB`); never soften an invariant to make a produce pass — fix the produced numbers.
...
**3b. Generative steps keep a candidate history.** For generative steps (Icon 2D, 3D, Material) a Produce run is a *re-roll*: each batch of candidates is **persisted, not discarded**, so an artist can A/B-compare across re-rolls and re-select an older candidate (Midjourney/Leonardo loop). The model is the pure `steps/shared/genHistory.ts` (`readHistory`/`makeBatch`/`appendBatch`/`selectCandidate`/`historyData`) stored under the artifact's `data.genHistory`; selecting a candidate projects its `payload` to top-level data so the existing `selected`/`tris`/`maps` Acceptance is unchanged. Render it with the shared `CandidateGallery` (each batch stamped with its direction + recoverable full prompt). Reference impl: `ItemArt.tsx` (`ItemIcon2D`/`Item3DGen`/`ItemMaterial`), candidate generators in `steps/shared/itemGenCandidates.ts`. **Real generated 2D art is automatic** for every `gallery` step: ArchetypeStep pre-fetches `GET /api/visual-gen/icons?catalogId=…&step=…`, the per-step library `scripts/gap-loop/*` writes to `generated/icons/` (served one file at a time by `/api/visual-gen/icon/<name>`, the same basename-allow-list shape as the 3D `/api/visual-gen/asset/<name>`), and feeds it to `steps/shared/imageGalleryCandidates.ts`. Matching is by **re-encoding** the artifact's identity with the generator's own filename rule (`@/lib/visual-gen/generated-icons.ts` → `iconSlug(catalogId, step[, entityId])`, names built with `iconFileBase`), so a step only ever shows art generated FOR it — never another step's, and never a mesh turntable. The key is `(catalog, entity, step)`: pass `&entityId=…` and an entity's own art wins, the per-step icon is the fallback, and each entry declares the `scope` that served it (see WIRING-AND-ACCEPTANCE.md). Only `min(candidates, assets)` slots carry a real image; the rest stay honest deterministic swatches, and a step with no art gets the full swatch fallback plus the "not the generated asset" caption. A step may still set `StepSpec.genCandidates` (`{ needsAssets?, assetKind?, build(direction, seq, assets) }`) for a bespoke generator — `assetKind: '3d'` is the opt-in that swaps the manifest for real `.glb` meshes (`meshGalleryCandidates`, `character-pipeline` 3D Generation).
...
**5b. Scope a step to canon profiles (rare).** A step that only makes sense for some canon profiles (e.g. `bestiary / Sprite Render`, a prerendered-sprite render for `diablo1`, meaningless for PoF's in-engine 3D monsters) declares `profiles: ['<profile>']`. There is no "not applicable" status: the step is simply **not part of** other profiles' pipelines. `src/lib/catalog/stepScope.ts` (`stepAppliesTo` / `stepsForProfile` / `stepLabelsForProfile`) is the one resolver — the lab rail and matrix (a `·` cell, never "pending"), lifecycle totals, the one-shot orchestrator, the e2e walker (which walks a PoF seed) and `scripts/diablo/status.ts` all go through it, and `submitStepArtifact` **refuses** a row for a step the entity does not have. Omit `profiles` for every ordinary step.
