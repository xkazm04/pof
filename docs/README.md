...
- **2026-07-23** — **Dual-execution program handbook**: [dual-execution-program.md](dual-execution-program.md) — 16 of ~31 pipelines reviewed with a browser duel (`kiro/saber-arpg`) + UE FeatureLab realizing the same `pipeline_artifacts` SOR; per-domain review recipe, runtime-evidence standards, and the remaining domains grouped for PARALLEL sessions (World/Characters/Narrative/Systems). Spec: [research/dual-execution-preview-spec.md](research/dual-execution-preview-spec.md); live registry `src/lib/preview/realization-facts.json` → /status B/U markers.
