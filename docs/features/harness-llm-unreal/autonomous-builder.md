...
- **Pause/resume** — same `runId` row across pause/resume (in-process AND across a server restart via rehydrate, see Durable run resume); the loop drains active sessions before stopping.
