...
- **Single biggest near-term unlock:** UE 5.7's full **MetaHuman Python/Blueprint scripting API** + UE 5.8's **"any mesh → MetaHuman" conforming**. Together they close PoF's long-standing auto-rig gap (UniRig was a placeholder) by routing generated character meshes into a fully-rigged, animation-ready MetaHuman. **Automatability (GROUND-TRUTHED on the installed 5.8.0, 2026-07-05):** the conform API is **fully Python-exposed and loads in a headless `-run=pythonscript -nullrhi` commandlet** — verified by introspection: `MetaHumanCharacterEditorSubsystem.conform_to_target_meshes` / `conform_body` / `set_body_mesh` / `get_face_model_coefficients` / `set_face_model_coefficients` (+ `MetaHumanIdentityFace.conform`). **The one catch:** the `MetaHumanCharacter` (Experimental) plugin must be enabled (`PoF.uproject` doesn't yet — enabling it takes the exposed MetaHuman-class count 108 → 243); install the MetaHuman Optional Content for full fidelity. So a **scripted HEADLESS conform is feasible** — the only residual manual step is a point-align fallback when auto-solve mis-detects topology (a quality gate, mitigated by input-mesh prep), not an API limitation. Concrete end-to-end recipe + probe repro: [`research/ai-to-metahuman-conform-recipe.md`](./research/ai-to-metahuman-conform-recipe.md).
...
**Automatability (GROUND-TRUTHED on the real 5.8.0 install, 2026-07-05):** the conform API is **fully Python-exposed AND loads in a headless `-run=pythonscript -nullrhi` commandlet** — verified by introspection: `unreal.MetaHumanCharacterEditorSubsystem.conform_to_target_meshes` (custom-mesh conform), `conform_body` / `set_body_mesh` / `set_body_joints`, `get_face_model_coefficients` / `set_face_model_coefficients`, `get_mesh_for_body_conforming{,_from_dna,_from_template}`, plus `MetaHumanIdentityFace.conform`. **The catch:** these live in the **`MetaHumanCharacter` (Experimental) plugin, which `PoF.uproject` does NOT enable** — with the default plugin set the API is absent (probe: 108 MH classes, `HAS_MetaHumanCharacter=False`); enabling it (`-EnablePlugins=MetaHumanCharacter`) surfaces the full subsystem (243 MH classes). So the earlier "editor-dependent, no commandlet" read (from the release-notes' silence) was **too pessimistic — a scripted HEADLESS conform is feasible.** Caveat: install the **MetaHuman Optional Content** or the plugin runs with "limited features". Budget a **manual point-align fallback** for auto-solve mis-detection (fused fingers → 4 markers on 2) — a QUALITY concern, mitigated by input-mesh prep (`UE_GOTCHAS` `metahuman-conform-input-prep` / `metahuman-conform-texture-export`) and/or a scripted `set_face_model_coefficients` correction.

**Effort / dependency:** Medium-High. **UE 5.8 IS installed locally** (`C:/Program Files/Epic Games/UE_5.8`; `PoF.uproject` EngineAssociation `5.8` with the `MetaHuman` + `MetaHumanBodyTracker` plugins enabled) — so this is **no longer install-blocked**; the real remaining dependencies are (a) driving the editor-attached conform via the PoF Bridge and (b) Asset Forge generation being wired (currently a placeholder endpoint — 🟡 in the roadmap). Next concrete step: prototype the scripted conform against a Hunyuan/Tripo character output and measure how often the manual point-align fallback fires. **Concrete recipe:** [`research/ai-to-metahuman-conform-recipe.md`](./research/ai-to-metahuman-conform-recipe.md).
