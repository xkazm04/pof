...
- **Single biggest near-term unlock:** UE 5.7's full **MetaHuman Python/Blueprint scripting API** + UE 5.8's **"any mesh → MetaHuman" conforming**. Together they close PoF's long-standing auto-rig gap (UniRig was a placeholder) by routing generated character meshes into a fully-rigged, animation-ready MetaHuman. **Automatability (verified 2026-07-05, research run):** almost every Creator op — sculpt, **conform**, wardrobe, **rigging**, texture synthesis — is now scriptable via Python/Blueprint (5.8 added `GetFaceModelCoefficients`/`SetFaceModelCoefficients` for conform-to-PCA + a batch-processing API + a `MetaHumanGenerator` MCP Toolset). **But it is scriptable *in-editor*, not confirmed fully headless** — Epic documents no commandlet path for the conform itself (only `ConvertLegacyDNAAssets` for DNA migration), and the observed 5.8 workflow needs a **manual point-align fallback** when auto-solve mis-detects topology (e.g. fused fingers). So: script it through the editor bridge, don't assume `-run=pythonscript` headless conform yet. Concrete end-to-end recipe: [`research/ai-to-metahuman-conform-recipe.md`](./research/ai-to-metahuman-conform-recipe.md).
...
**Automatability (verified 2026-07-05):** 5.8 conform *is* scriptable via the MetaHuman Python API (`Get/SetFaceModelCoefficients` conform-to-PCA + the batch API) but is **editor-dependent, not a documented commandlet** — dispatch it through the PoF Bridge (editor-attached Python), not a headless `-run=pythonscript` commandlet. Budget for a **manual point-align fallback**: auto-solve mis-detects complex topology (fused fingers → 4 markers on 2 fingers) and needs Reset Body + hand-placed solve points; the input-mesh prep (A-pose, separated fingers, hair removed, high-poly head) is what keeps that fallback rare — captured as `UE_GOTCHAS` `metahuman-conform-input-prep` / `metahuman-conform-texture-export`.

**Effort / dependency:** Medium-High, and **gated on 5.8 stability** (preview, not quality-tested) — and 5.8 is **not installed locally** (engine targets 5.7), so this is currently *blocked on a live 5.8 editor* to verify the scripted conform end-to-end. Prototype against preview, productionize when 5.8 ships. Also depends on Asset Forge generation being wired (currently a placeholder endpoint — 🟡 in the roadmap). **Concrete recipe:** [`research/ai-to-metahuman-conform-recipe.md`](./research/ai-to-metahuman-conform-recipe.md).
