...
# Make combat timelines touch + keyboard scrubbable
...
- **Generated**: 5/24/2026, 11:44:26 AM

## Description
TimelineBar in the dodge editor and the choreography TimelineSection scrub via raw onMouseDown plus window mousemove and mouseup, so they only respond to a mouse. Refactor to Pointer Events with setPointerCapture so touch and pen work, add role=slider with aria-valuemin, aria-valuemax and aria-valuenow, and support arrow keys to nudge one frame at 60fps plus Home and End to jump to the timeline ends.

## Reasoning
Scrubbing frame data is the signature interaction of this context, yet it is unusable on tablets and for keyboard-only users. Pointer events and slider semantics make the marquee feature work for everyone with no change to the visual design.
