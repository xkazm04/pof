...
# Stat-Weight & Tuning Sensitivity Sweep
...
- **Generated**: 5/24/2026, 12:29:14 PM

## Description
Add a sweep mode that steps one tuning param (e.g. enemyDamageMul) or one player stat across a range, re-runs runCombatSimulation at each step, and plots survival/DPS/duration curves so designers see exactly where balance breaks. Push it further than the originals by auto-recommending the value that hits a target survival rate (a goal-seek), reusing buildBuckets and the existing /api/combat-simulator POST.

## Reasoning
Scaling/stat-weight charts are the power-user core of Path of Building and SimulationCraft � they convert blind slider-twiddling into data-driven tuning by revealing the response curve and the sweet spot. This directly serves the context purpose (tune frame data, attack lanes, DPS) and gives designers an objective answer instead of trial and error.
