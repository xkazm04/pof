...
# Unify divergent stat cards into one MetricCard
...
- **Generated**: 5/24/2026, 12:01:53 PM

## Description
Four separate stat-card components live in this one context: a bespoke motion card in DirectorOverview.tsx, a SurfaceCard-based horizontal card in RegressionTrackerView.tsx, and two thin wrappers around KPICard in SessionAnalyticsDashboard.tsx and WeeklyDigestView.tsx. Build a single MetricCard primitive (props: layout, accent, icon, label, value, optional delta, animate) and replace all four call sites so padding, radius (rounded-xl), and entrance motion stay identical everywhere.

## Reasoning
Four implementations of the same concept guarantee visual drift and slow every future change. A single primitive enforces one rhythm of spacing and shadow across the whole telemetry surface and becomes reusable app-wide. Blast radius is low since the markup is already nearly identical.
