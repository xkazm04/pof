...
# Closed-Loop Balance Autopilot for Genomes
...
## Description
Ingest live playtest telemetry from the running UE5 build (per-archetype win rate, time-to-kill, death frequency) and feed it back into the Character Genome editor. Designers set a balance target like 'all archetypes within 10% win rate' and the system flags drifting genomes and proposes specific profile adjustments to converge toward it. First step (one sprint): define a BalanceTarget schema plus a /api/telemetry combat-outcome ingest path and render a 'balance drift' badge beside each genome in GenomeSelectorBar.

## Reasoning
Today the genome editor is balance-blind: values are set by intuition against mock curves with zero feedback from real play. A self-correcting loop turns a static authoring tool into a living balance autopilot, the signature capability that defines a category and would make designers tell their friends about it.
