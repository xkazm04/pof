...
# Unify combat sliders into accessible RangeSlider
...
- **Generated**: 5/24/2026, 11:44:23 AM

## Description
ParameterEditor, the CombatActionMap feedback tuner, and the choreography TuningPanel each hand-roll a slider: an opacity-0 range input layered over a custom neon track with a pointer-events-none thumb. Extract a shared RangeSlider primitive that keeps the glowing track and thumb but adds a focus-visible ring on the thumb (ring-2 ring-offset-1 in the param color), an aria-label plus aria-valuetext, and shared gap and sizing tokens. Replace the three bespoke implementations.

## Reasoning
Keyboard users currently get no visible focus and screen readers get no value context because the real input is invisible and unlabeled. One primitive fixes accessibility everywhere at once and removes duplicated slider markup across the context.
