...
# Premium, error-proof simulation config controls
...
- **Generated**: 5/24/2026, 12:21:23 PM

## Description
The two config panels diverge and feel cramped: ItemEconomySimulator uses tiny w-16 number inputs that only clamp to min (parseInt || min, no upper bound), while EconomySimulatorView clamps both ends. Replace bare number fields with a shared ConfigControl that pairs a styled range slider (accent-colored track, focus-visible ring) with a live tabular-nums readout and +/- stepper buttons, and clamps to [min,max] in React state for both panels. Group controls with consistent gap-3 and aligned labels.

## Reasoning
Sliders make tuning feel tactile and premium while making out-of-range input impossible (error prevention), and unifying the two panels removes a jarring inconsistency where the same task looks and behaves differently. Direct quality-of-life win for the core configure-then-simulate loop.
