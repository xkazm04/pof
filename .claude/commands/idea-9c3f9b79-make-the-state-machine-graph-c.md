...
# Make the state-machine graph canvas keyboard accessible
...
## Description
In StateMachineEditor.tsx, state nodes are click/drag-only <div>s and transitions are SVG <line>s with onClick, so none are reachable by keyboard or announced to screen readers. Add tabIndex/role and an aria-label per node (name, priority, flag), a visible focus-visible ring, arrow-key nudging of node x/y position, Enter/Space to select, and a keyboard path to draw a transition from the selected node.

## Reasoning
The editor is the centerpiece of this context yet is unusable without a mouse and invisible to assistive tech. Keyboard plus ARIA support makes a core authoring tool usable by everyone and is the baseline expectation for a premium node editor.
