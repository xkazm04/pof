...
# Ask-your-game: plain-language producer over the matrix
...
## Description
An always-on conversational Producer that translates the entire feature matrix + dependency state into answers a non-technical creator understands: "Is my game playable yet?", "What is stopping combat from working?", "What should I build this weekend?". It grounds Claude answers in the aggregate matrix/NBA/blocker JSON and returns a plain-language status, the human reason behind each blocker, and a weekend-sized roadmap � no jargon like GAS or AnimBP. First sprint step: add a Project Q&A box to CrossModuleFeatureDashboard that POSTs the aggregate + blocker data with a curated question to Claude and renders a plain-language "what is left to play the game" summary.

## Reasoning
The matrix is dense with engineering jargon that excludes the exact day-to-day non-technical users the Globalization goal targets. A natural-language producer makes the whole system legible to anyone, turning a developer dashboard into a creative copilot people tell their friends about.
