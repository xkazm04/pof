...
# Encounter & Combo Profile Library (save/load)
...
- **Generated**: 5/24/2026, 12:29:02 PM

## Description
Persist named encounters (CombatChoreographyEditor enemy layouts, waves, tuning) and custom combos (ComboChainBuilder chains) to the project SQLite DB via a new *-db.ts module, surfaced as a sidebar list with rename/duplicate/delete. Today all editor state is ephemeral useState and combos are limited to hardcoded PRESET_COMBOS, so nothing survives a refresh. Reuse the existing combatSimulatorStore + apiSuccess/apiError envelope pattern to add CRUD endpoints.

## Reasoning
Profile management is the foundational, table-stakes feature of every serious theorycrafting tool (SimulationCraft profiles, Path of Building build saves, Linear saved views) � without it designers cannot iterate across sessions or compare past work. It is also the prerequisite that unlocks comparison, sharing, and templates. High value for moderate effort given the DB and store scaffolding already exist.
