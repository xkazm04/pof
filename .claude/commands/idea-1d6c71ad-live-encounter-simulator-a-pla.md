...
# Live Encounter Simulator: a playable AI sandbox
...
## Description
Fuse the currently-static PerceptionConeViz, BTFlowchart, TacticsMap, PatrolPointsDistribution and Wave Choreographer into one deterministic, tick-based simulation that actually plays an encounter in the browser. The behavior tree ticks and highlights its live active node, perception genuinely detects a moving player, the AI Director (squad-tactics EQS composition types already defined) positions enemies, and waves spawn on a scrubable timeline. Designers tune an encounter and watch it resolve before ever opening UE.

## Reasoning
Today every visualization is a hardcoded diagram; turning them into one live simulation is a 10x leap from illustration to playground and would become the bestiary tab everyone demos. It collapses the slow design-in-UE / rebuild / observe loop into seconds and makes the app the definitive pre-production AI design surface.
