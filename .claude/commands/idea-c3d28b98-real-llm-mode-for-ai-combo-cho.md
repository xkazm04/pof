...
# Real LLM mode for AI Combo Choreographer
...
- **Generated**: 5/24/2026, 11:54:07 AM

## Description
The Combo Choreographer is branded AI but generateCombo() is pure keyword matching plus seeded random, while the app already ships a Claude CLI task system (useModuleCLI, TaskFactory, callback markers). Add a Design with Claude toggle that dispatches the natural-language prompt through a new TaskFactory method whose callback returns a validated GeneratedCombo JSON, keeping the deterministic generator as an instant offline fallback.

## Reasoning
Users coming from Copilot and ChatGPT expect a tool labelled AI to actually reason about intent like a defensive parry-into-riposte that staggers on the second hit, which keyword maps cannot. The hybrid pattern (instant local draft, optional LLM refine) is proven in modern AI editors and reuses infrastructure that already exists, so the value-to-effort ratio is strong.
