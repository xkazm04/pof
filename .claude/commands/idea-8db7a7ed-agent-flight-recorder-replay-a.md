...
# Agent Flight Recorder: Replay and Branch Any Run
...
- **Generated**: 5/24/2026, 6:17:03 PM

## Description
Persist every execution event stream (prompts, tool calls, file changes, callbacks, UE asset diffs) from cli-service.ts into SQLite instead of memory, then build a scrubbable timeline UI that lets users rewind to any checkpoint, fork the run with a tweaked prompt, and compare outcomes side-by-side � git for AI agent runs. The events array, log files, and dispatchHealth signatures already capture the raw material; this makes it durable, queryable, and branchable.

## Reasoning
Agent runs are opaque black boxes today � when a 40-minute dispatch hangs or produces wrong assets, there is no way to inspect, replay, or branch from the divergence point. A flight recorder turns the CLI into a debuggable, version-controlled creative process, building trust and enabling experimentation that no rival agent offers.
