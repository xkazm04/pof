...
# Round-Trip C++ to Genome Living Sync
...
## Description
Make codegen bidirectional: parse the generated AARPG*Character .cpp and AttributeInitTable CSV back into a CharacterGenome, detect drift between the authored genome and what is actually committed in the UE5 source, and offer a 3-way merge so hand-edits and editor edits reconcile. First step (one sprint): write a parser that extracts movement/combat constructor values from a generated .cpp into a partial genome and renders a field-level diff against the stored genome.

## Reasoning
Codegen is one-way today, so the moment an engineer hand-tweaks MaxWalkSpeed in C++ the genome is permanently stale and trust in the tool erodes. Bidirectional sync makes the genome the durable single source of truth that tracks the live codebase, effectively version control for game balance.
