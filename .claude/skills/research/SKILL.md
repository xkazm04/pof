...
Extract 5–15 candidates. Each: `title` (imperative, <60 chars), `summary` (1-2 sentences), `source_anchor` (≤20-word quote or `[HH:MM:SS]`), `bucket`, `effort`, a one-line **impact-map area** (which subsystem it'd touch — read straight from the already-loaded `docs/research/impact-map.md`; cheap), and the mandatory **`status_impact`** (see the lens above — which /status cell class or pipeline step(s) it could improve, or `none`). Apply memory filters (deprioritize patterns the user repeatedly declines; surface matching `descoped-reopenable` items as "reconsider?") + source-type yield calibration (engineering talk = dense; product/competitor demo = few + many "already-have" catches). Quick relevance: drop only candidates with **no plausible PoF attachment point**. **Do NOT web-search, grep, or read anchor files yet** — that budget is spent only on what the user picks. **One exception — a NUMERIC premise about artifacts that already exist locally.** If a candidate claims something measurable about produced output ("generated meshes are the wrong size", "bakes are untextured", "clips are N frames") and the artifacts sit in `generated/` (or the UE project), measure them NOW with one loop before the gate — a `trimesh` extents pass over every `.glb` took seconds on 2026-08-17 and turned "scale might be off" into "every asset is 1.0 m, provably", and the captured numbers became the test fixtures. A measured premise is a real candidate; an unmeasured one is a guess the user is asked to rank.
...
1. **Web-augment (bounded: ≤3 `WebSearch`/`WebFetch` calls for a single-topic source, or ≤2 PER PICKED CANDIDATE for a multi-topic one)** — only if a pick names a tool/technique/model whose definition sharpens framing and isn't already in the impact-map/`docs/`. A **news roundup** names a dozen unrelated products, and each pick is its own question, so a flat per-run budget silently forces you to ship one pick unverified; scale it with the picks and say at the gate which budget you are on. **Verifying the SOURCE'S OWN claims is inside the budget, not outside it.** On 2026-09-07 the video asserted a paper's code was "already released"; one fetch showed the project page reading *Code (coming soon)* — no repo, no licence — which flipped that finding from a build to a standing descope. A roundup's confident factual claims about releases, licences and availability are the single highest-yield thing to check, and they are exactly what gets cut first when the budget is tight. One focused search → fetch the most authoritative page; capture a 2-4 sentence note (what / how / integration shape / why it matters to PoF). Don't validate the speaker's claims here; don't rabbit-hole.
2. **Impact analysis (anti-re-scan):** impact-map first → **host-infrastructure-first grep** (MCP tool → `Register*Routes`/registry; harness step → `src/lib/harness/`; pipeline step → `getStepComponent`/`ArchetypeStep`; API route → `apiSuccess`) → **read the anchor file(s)** (~30-100 lines) for the exact `file:line`.
   ...
   **When the practice is ALREADY in the knowledge store, the question is not "should we know this" — it is "does the code OBEY it?"** A source whose lesson is already a shipped gotcha reads like a duplicate and gets dropped as an already-have catch; that reflex hides the higher-value finding, because a knowledge entry and the code path it governs are written by different runs and drift apart silently — the entry renders into every prompt while the route keeps doing the opposite. 2026-09-07: the video's core lesson ("generator low-poly modes wreck complex assets; generate dense and retopo") was word-for-word the shipped `ai-lowpoly-generation-not-final` gotcha — and `/api/visual-gen/generate` was handing EVERY class a `face_limit`, i.e. making exactly the request the gotcha forbids. So when you catch an already-have on a KNOWLEDGE entry, spend one more grep: find the code path that entry is about and check it complies. A knowledge/code divergence is a real defect, it is invisible to both the golden rail (the prompt is correct) and the test suite (the code is self-consistent), and only this comparison finds it.
...
- **In-repo, S/M** → implement with **TDD** (`superpowers:test-driven-development`) + an atomic `research:` commit per finding. **Validate before commit:** the affected test(s) + `tsc --noEmit` + `eslint` on changed files for an S/M single-area change; run full `npm run validate` for an L or multi-area change. **"Affected tests" INCLUDES any golden/snapshot suite that pins the data you edited** — a knowledge entry in `ue-gotchas.ts` / `module-eval-prompts.ts` is rendered into every task prompt, and `src/__tests__/lib/prompts` pins those prompts. On 2026-08-17 the golden rail was found RED on HEAD: the 08-09 and 08-12 research commits had each added a gotcha, run only the gotcha test, and shipped. Run the golden suite; if the drift is exactly your entry, re-record it intentionally (`POF_UPDATE_GOLDEN=1`) and say so in the commit. **Read WHICH goldens drifted — that list IS the consumer census for a knowledge entry, and a stronger one than a grep.** A gotcha or criterion has no importer to find; its reach is the set of rendered prompts that changed, and the rail names them for free. So a knowledge entry that drifts ZERO goldens is the dead-store defect arriving by a different door: it compiled, its own test passes, and it reaches no prompt. Treat that as a routing failure and find a store that is actually rendered, rather than committing an entry whose only consumer is itself. (2026-09-07: a creature-rigging gotcha drifted exactly `task-mixamo-import` and `task-character-setup` — the same two prompts a reviewer would want named, obtained without a single extra command.)
...
**When a finding ships a PURE module, check it against a hand-authored answer ALREADY IN THE REPO — not against its own opinion.** The artifact-diff rule above has no purchase on a planner, a mapper, a scorer or a normalizer: there is no output file to diff, so the fallback is a suite of cases you invented, which proves only that the module agrees with itself. That is the same blind spot as the imagined fixture, one level up. Before writing those cases, look for a place where a human already wrote the correct answer down — a hand-maintained mapping table, a golden config, a preset someone tuned, a constant with a comment explaining the value — and assert your derivation reproduces it. 2026-09-07: a bone-name conform planner was checked against `rig-presets.ts`, which carries a hand-authored 20-bone Mixamo→UE5 mapping, and reproducing it **20/20 from names alone** is evidence no self-written case could give. It also paid immediately: the check is what exposed a matching pass that looked like the confident one (equal chain indices) and silently shifted an entire bone chain by one, because the two conventions disagree on where a chain starts. Where no such answer exists in the repo, say in the summary that the module is verified only against constructed cases — and prefer a ground truth that predates your change, since one you wrote this session is just your opinion in another file.
...
**Before you tag a shipped item for scoped delivery, READ THE SCOPE TABLE — an empty entry excludes everything.** The same failure as a defaulted parameter, arriving one step earlier: where a corpus routes items to consumers by a tag (`modules`, `appliesTo`, an area label), the routing table can map the very consumer you are targeting to an EMPTY list, and a filter of the form "universal items, plus those whose tags intersect this consumer's" then admits your tagged item never. 2026-09-07: `MODULE_GOTCHA_DOMAINS` maps `'multiplayer'`, `'physics'`, `'save-load'` and `'dialogue-quests'` to `[]`, so following the corpus's own tagging convention would have made three new replication/lifecycle/persistence gotchas unreachable from the four modules that need them most — invisibly, since the entries render fine for every other module. Open the routing table and check your target's row before tagging; when the row is empty or the item is genuinely cross-cutting, leave it untagged and pin the reach with a test. Then let the golden drift confirm it: the PER-FILE drift counts are a free routing check, not just a census — 3 lines into each `ue-cpp` prompt and exactly 1 into each of the four `ue-python` prompts proved each entry landed in the RIGHT prompts, which a bare "35 files changed" would not have.

**The census has ONE blind spot, and it is defaulted parameters — so check the CALL SITE, not only the import.** A module can be imported by the wired path and still have its substance inert, because any optional argument with a sane fallback silently collapses a rich data structure into a single constant. 2026-09-07: a run shipped a material-density table, passed the census on every symbol (the route imported them all), and its live output came back `phys_default` for **every** prop — the route called `physicalForSize(size)` and never passed a material, so the whole table was unreachable in the only path that ran. Green tests, clean census, dead data. After the import check, for each data structure you shipped ask *which argument selects into it* and grep the call site for that argument; if the wired caller never passes it, the table is decoration. The cheapest proof remains a live run whose real output you PRINT — the defaulted parameter is invisible to every fixture that supplies it explicitly.
...
**Close every run by verifying each shipped symbol is actually in `HEAD`** — `git grep <symbol> HEAD -- <path>` per finding, not `git log`. A concurrent session can rewrite history and drop your commit's content back into the working tree, where **the tests still pass and nothing looks wrong**: green tests prove the working tree, never the commit. On 2026-08-14 this sweep was the only thing that caught a shipped finding sitting uncommitted after a parallel rebase.
