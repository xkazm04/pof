 ...
 */
import { type ToolDef, readOnly } from './shared.js';
...
export const GROUPS_TOOL: ToolDef = {
  ...
  // Reads the group table and one env var inside this process — it touches no backend,
  // no editor and no disk, so it is the most auto-approvable tool the server has.
  annotations: readOnly('Tool groups'),
  description:
