...
- Substrate materials may have higher baseline cost — profile vs legacy`,
  ...
  audio: {
    focus: 'Sound cues, attenuation/spatialization, submix routing, VO and dialogue speech, voice assignment, audio asset licensing',
    structureChecks: `- Sound assets should be referenced as USoundBase/USoundCue soft references on data assets, never as hardcoded /Game/ path strings in C++
- Every speaker (NPC, player, narrator) must have a voice binding declared in data — a speaking character with no assigned voice is an unplayable line, and the binding belongs beside the character definition, not inside the dialogue node
- VO assets should be keyed off the same localization key as their subtitle, so a line, its caption and its audio cannot drift apart
- Attenuation and concurrency settings belong on shared USoundAttenuation/USoundConcurrency assets, not duplicated per emitter
- Submixes should be split by category (music / SFX / ambient / voice) so voice can be ducked and volume-controlled independently
- Generated or cloned speech must record BOTH licence axes on the asset: the model weights' licence AND the provenance of the reference voice sample. A permissively-licensed model does not make a cloned voice redistributable — the voice inherits the terms of whatever produced the reference clip, and an "Apache weights" claim says nothing about the sample they were conditioned on`,
    qualityChecks: `- Voice-cloning reference samples must have a recorded, redistributable source; a sample generated by a commercial TTS service carries that service's terms into every clip cloned from it, regardless of the local model's licence
- Non-commercial model licences (CC-BY-NC, research-only, CPML) must be flagged as unshippable rather than silently used — the licence is a property of the shipped audio, not of the dev-time convenience
- A declared VO/SoundCue asset path must correspond to audio that is actually produced; a step that emits VO *text* while declaring SoundCue assets is claiming a deliverable it did not create
- Dialogue audio should stream rather than fully load, and one-shot barks should not block on a synchronous load
- Runtime-generated speech needs a stated latency budget and a fallback: if generation exceeds the budget, the line should degrade to subtitle-only rather than leaving the NPC mute
- Speech generated at runtime must be pipelined at sentence granularity (begin synthesis on the first complete sentence rather than waiting for the full response) or the perceived latency is the sum of both models, not the larger of them
- Audio must not be the only channel for gameplay-critical information (accessibility): every audio cue that gates progress needs a visual counterpart`,
    performanceChecks: `- Concurrency limits should cap simultaneous voices per category so a crowd cannot exhaust the voice pool
- Distant or inaudible emitters should be virtualized rather than rendered
- Large VO banks should be soft-referenced and loaded per-scene, not resident for the whole game
- Audio components on pooled or frequently spawned actors should be reused, not created per spawn
- Compressed formats and appropriate sample rates should be used for VO; full-rate uncompressed speech is a common package-size regression`,
  },
  'ai-behavior': {
...
- NPC lines must be consistent with world state: an NPC should reference a fact only when the world state says that fact is established and known to them, and must never assert a fact that contradicts world state. Each speaker needs a bounded knowledge set (what they know, what they cannot know) — an NPC who volunteers a secret they were never told is the same defect as one who denies an event the player just completed
- Facts shared between speakers must agree: when two NPCs both reference the same world event, their accounts must not contradict each other in detail or in tense, and a fact retracted by world state must stop being spoken by everyone
- Generated or templated dialogue must be grounded in named world-state entries rather than invented detail — a line that introduces a place, faction or character absent from the world data is unverifiable content, and should fail rather than pass as flavour`,
