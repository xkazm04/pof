...
export const ARPG_CHECKLISTS: Record<string, ChecklistItem[]> = {
  'arpg-character': [
    {
      id: 'ac-1',
...
List any issues found and fix them. Verify build compiles.`,
    ...
    {
      id: 'ac-6',
      label: 'Verify character locomotes (humanoid, natural pose)',
      description: 'Launch the slice and confirm the player (and enemy) render as humanoids in a natural pose — not T-posed — via an agentic screenshot + Gemini vision check.',
      prompt: `Confirm the character is correctly set up and animating, then verify it actually renders on screen.

1. Ensure the player pawn's Mesh has a non-null SkeletalMeshAsset AND a live AnimInstance — e.g. the MoverTests SKM_Manny driven by ABP_Manny, or a custom AnimBP. The standard mannequin mesh uses the relative offset (0,0,-90) with yaw -90.
2. If an enemy is present, make sure it is visually distinct — a strongly-contrasting material such as M_EnemyRed, NOT a subtle mannequin material instance.
3. Build, then launch the slice so the character stands/idles in a natural pose. A T-pose (arms straight out) means the AnimInstance is not driving the mesh — fix the AnimBP/mesh wiring before relying on the visual check.`,
      characterCheck: true,
    },
  ],
  'arpg-animation': [
...
SLICE MODE (vertical slice — no inventory): For the minimal slice path, instead of spawning a pickup that requires UARPGInventoryComponent on the player, spawn the AARPGWorldItem with a Lifetime of 60 seconds (InitialLifeSpan = 60.0f) so it self-destructs automatically. Do not call any inventory API; do not require the player to have an inventory component.` },
...
SLICE MODE (vertical slice — no inventory): The primary slice variant is overlap-destroy: on overlap, spawn a small "+gold" Niagara effect at the pickup location and Destroy() the actor. No item storage, no inventory component required. The full inventory variant remains documented as a LATER: follow-up.` },
  ...
  'arpg-ui': [
    { id: 'au-1', visualCheck: true, label: 'Set up HUD framework', description: 'Create main HUD widget with health/mana bars, ability bar, and minimap placeholder.', prompt: `Create the main HUD widget for my aRPG as a pure-C++ widget extending UARPGCodeWidgetBase (override BuildTree(); the base's RebuildWidget() override calls it, and it supplies CreateStyledProgressBar/CreateStyledTextBlock/AnchorTopLeft/AnchorTopCentre helpers) — no companion Widget Blueprint, and do not use BindWidget. Build the widget tree in RebuildWidget() (before Super::RebuildWidget()), NOT in NativeConstruct() (which runs after the Slate tree is built, so WidgetTree mutations there do not render). Layout: top-left player health bar (green) and top-centre enemy/target health bar (red), each with numeric values. Style every UProgressBar with an explicit FProgressBarStyle — a dark track (BackgroundImage) and a bright fill (FillImage) — because an empty ProgressBar is invisible with the engine default. Bind to GAS attribute-change delegates (GetGameplayAttributeValueChangeDelegate) for real-time updates. Use UVSHUDWidget as the reference implementation.` },
    ...
    { id: 'au-3', visualCheck: true, label: 'Create floating enemy health bars', description: 'UWidgetComponent on enemies showing health bar, enemy name, and level.', prompt: `Create the floating enemy health bar as a pure-C++ widget extending UARPGCodeWidgetBase (override BuildTree(); the base's RebuildWidget() override calls it, and it supplies CreateStyledProgressBar/CreateStyledTextBlock/AnchorTopLeft/AnchorTopCentre helpers) — no companion Widget Blueprint, and do not use BindWidget. Build its widget tree in RebuildWidget() (before Super::RebuildWidget()), NOT in NativeConstruct() (which runs after the Slate tree is built). Style the UProgressBar with an explicit FProgressBarStyle — a dark track (BackgroundImage) and a bright fill (FillImage) — because an empty ProgressBar is invisible with the engine default. Host the widget on a UWidgetComponent attached to enemy characters, set to Screen space. The widget shows: enemy name, level number, and the health bar that updates on damage. The bar fades in when the enemy takes damage and fades out after 3 seconds of no damage. Hide for dead enemies.` },
    { id: 'au-4', visualCheck: true, label: 'Implement ability cooldown UI', description: 'Ability bar slots show icon, cooldown sweep overlay, mana cost, and keybind label.', prompt: `Implement the ability bar as a pure-C++ widget extending UARPGCodeWidgetBase (override BuildTree(); the base's RebuildWidget() override calls it, and it supplies CreateStyledProgressBar/CreateStyledTextBlock/AnchorTopLeft/AnchorTopCentre helpers) — no companion Widget Blueprint, and do not use BindWidget. Build the slot tree in RebuildWidget() (before Super::RebuildWidget()), NOT in NativeConstruct(). Create 4 ability slots along the bottom-centre. Each slot shows: ability icon (UTexture2D from ability data), a cooldown sweep overlay (radial wipe that counts down), the mana cost number, and the keybind label (1/2/3/4). Bind to GAS ability cooldown tags to drive the sweep animation. Anchor the bar at Z-order 30 so it sits above engine debug text.` },
    { id: 'au-5', label: 'Create inventory screen', description: 'Grid-based inventory UI with item icons, rarity borders, tooltips on hover, drag-and-drop.', prompt: `[SLICE: skip — requires arpg-inventory (out of scope for the vertical slice)]

Create the inventory screen UI for my aRPG. Grid layout (6 columns x 5 rows) of item slots. Each slot shows: item icon, stack count badge, rarity-colored border. On hover: show tooltip with item name, description, stats, affixes, rarity. Implement drag-and-drop between inventory slots. Add equipment panel on the left showing paper doll with equipment slots.` },
...
Create a character stats panel for my aRPG. Show all attributes from the AttributeSet: Health/MaxHealth, Mana/MaxMana, Strength, Dexterity, Intelligence, Armor, AttackPower, CriticalChance, CriticalDamage. For each, show base value + bonus from equipment in a different color. Add a section showing damage reduction percentage calculated from Armor.` },
    { id: 'au-7', visualCheck: true, label: 'Implement floating damage numbers', description: 'Spawn floating text at hit location showing damage amount, colored by type, with animation.', prompt: `Implement floating damage numbers for my aRPG. Reference pattern: a UDamageNumberManagerComponent on the player controller auto-subscribes to a global damage delegate and spawns a UDamageNumberWidget per hit. Make UDamageNumberWidget a pure-C++ widget extending UARPGCodeWidgetBase — build its tree in RebuildWidget()/BuildTree() and mark the text block UPROPERTY(meta=(BindWidgetOptional)) so it works without a Widget Blueprint. When damage is dealt: spawn at the hit location in screen space, color by type (white=physical, red=fire, blue=ice, yellow=lightning), float upward and fade out over 1 second. Critical hits show larger text with a "CRIT!" prefix; heal numbers show green with a "+" prefix.` },
    { id: 'au-8', visualCheck: true, label: 'Create pause and settings menus', description: 'Pause menu with Resume, Settings (audio/video), Save, Quit. Settings save to GameUserSettings.', prompt: `Create the pause menu as a pure-C++ widget extending UARPGCodeWidgetBase (override BuildTree(); the base's RebuildWidget() override calls it, and it supplies CreateStyledProgressBar/CreateStyledTextBlock/AnchorTopLeft/AnchorTopCentre helpers) — no companion Widget Blueprint, and do not use BindWidget. Build the button tree in RebuildWidget() (before Super::RebuildWidget()) and wire each UButton's OnClicked delegate in C++. On ESC press: pause the game (SetGamePaused), show buttons: Resume, Settings, Save Game, Quit to Main Menu. Settings screen with tabs: Graphics (resolution, quality presets, vsync, frame limit), Audio (master/music/sfx volume sliders), Controls (show keybindings). Persist settings using UGameUserSettings.` },
  ],
  'arpg-progression': [
...
- Root motion: verify extracted root motion matches original movement distances.` },
  ...
  // Game Systems
  {
    ...
    { id: 'ai-5', label: 'Add group AI coordination', description: 'Implement squad manager for flanking, surrounding, and focus-fire behaviors.', prompt: 'Create a group AI coordination system for my UE5 project. Create UAISquadManager (UWorldSubsystem): tracks groups of AI (TMap<FName, TArray<AAIController*>>). When 3+ enemies target same player: assign roles (1 engager attacks from front, 1-2 flankers use FindFlankPosition EQS, rest circle at range). Implement melee positioning as a RESERVATION system owned by each target (the WildStar pattern): the target manages a ring of sized attack slots (man-sized ~4 — front/back/flanks; large creatures 8-16; whatever fits the silhouette), melee attackers RequestReservation(AIController) and receive a specific slot, must move there and CONFIRM within a timeout or the slot is reclaimed. Large attackers request ADJOINING slots — if two free slots are not adjacent, the target asks a smaller holder to reseat. Mass-cancel every reservation when the target dies, jumps far, teleports, or takes flight (holders fall back to ranged attacks or retarget); re-offer slots on landing. Cap simultaneous attackers per target (e.g. 2 active swings) so others hold position at their slot. On ally death: reassign roles. On target switch: redistribute group. Add debug draw to visualize squad roles, slots, and reservations. This prevents "surround and stunlock" and enemies stacking inside each other.' },
  ...
  },
  {
    ...
    checklist: [
    { id: 'ih-1', label: 'Define Enhanced Input Actions', description: 'Create UInputAction assets for all gameplay inputs with correct value types.', prompt: `Create Enhanced Input UInputAction assets for the vertical slice. Primary slice actions: IA_Move (Axis2D, for WASD movement) and IA_Attack (Digital/bool, for LMB). Place them in Content/Input/Actions/. Use the Enhanced Input plugin (already enabled in UE 5.7).
...
LATER: For non-slice scope, add IA_Jump (Digital), IA_Interact (Digital), IA_PrimaryAbility through IA_QuaternaryAbility (Digital), IA_Dodge (Digital), IA_Sprint (Digital), IA_Pause (Digital), IA_ToggleInventory (Digital). Skip these for the vertical-slice build — they are not exercised by the slice scenario.` },
    { id: 'ih-2', label: 'Create Input Mapping Context', description: 'Set up UInputMappingContext with keyboard+mouse and gamepad bindings.', prompt: `Create IMC_Default (UInputMappingContext) and add the slice actions to it. Map IA_Move to WASD using a 2D Axis modifier (Negate on W/D vs S/A as appropriate); map IA_Attack to Left Mouse Button with no modifiers. Place it in Content/Input/.
...
In AARPGPlayerController::OnPossess, add IMC_Default to the EnhancedInputLocalPlayerSubsystem with priority 0.` },
  ...
  },
  {
    ...
    knowledgeTips: [
      ...
      { title: 'Headless Blender for batch authoring; MCP socket for interactive editing', content: 'Two ways to drive Blender from PoF. Headless (`blender --background --python <script>`) runs a one-shot batch and is the most robust path for unattended authoring (arena build, per-room-template FBX export) — it is the default for dispatch jobs. The BlenderMCPService TCP socket keeps a live Blender session open for interactive iteration — use it when an operator is at the PoF UI adjusting a mesh and wants to see the change immediately. Rule of thumb: scripted one-shot → headless; live/interactive editing → socket.', source: 'best-practice' },
    ],
    checklist: [
