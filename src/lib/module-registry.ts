...
SLICE MODE (vertical slice — no inventory): The primary slice variant is overlap-destroy: on overlap, spawn a small "+gold" Niagara effect at the pickup location and Destroy() the actor. No item storage, no inventory component required. The full inventory variant remains documented as a LATER: follow-up.` },
  ...
  'arpg-ui': [
    ...
    { id: 'au-3', visualCheck: true, label: 'Create floating enemy health bars', description: 'UWidgetComponent on enemies showing health bar, enemy name, and level.', prompt: `Create the floating enemy health bar as a pure-C++ UUserWidget — no companion Widget Blueprint, and do not use BindWidget. Build its widget tree in RebuildWidget() (before Super::RebuildWidget()), NOT in NativeConstruct() (which runs after the Slate tree is built). Style the UProgressBar with an explicit FProgressBarStyle — a dark track (BackgroundImage) and a bright fill (FillImage) — because an empty ProgressBar is invisible with the engine default. Host the widget on a UWidgetComponent attached to enemy characters, set to Screen space. The widget shows: enemy name, level number, and the health bar that updates on damage. The bar fades in when the enemy takes damage and fades out after 3 seconds of no damage. Hide for dead enemies.` },
...
Create a character stats panel for my aRPG. Show all attributes from the AttributeSet: Health/MaxHealth, Mana/MaxMana, Strength, Dexterity, Intelligence, Armor, AttackPower, CriticalChance, CriticalDamage. For each, show base value + bonus from equipment in a different color. Add a section showing damage reduction percentage calculated from Armor.` },
    ...
    { id: 'au-8', visualCheck: true, label: 'Create pause and settings menus', description: 'Pause menu with Resume, Settings (audio/video), Save, Quit. Settings save to GameUserSettings.', prompt: `Create the pause menu as a pure-C++ UUserWidget — no companion Widget Blueprint, and do not use BindWidget. Build the button tree in RebuildWidget() (before Super::RebuildWidget()) and wire each UButton's OnClicked delegate in C++. On ESC press: pause the game (SetGamePaused), show buttons: Resume, Settings, Save Game, Quit to Main Menu. Settings screen with tabs: Graphics (resolution, quality presets, vsync, frame limit), Audio (master/music/sfx volume sliders), Controls (show keybindings). Persist settings using UGameUserSettings.` },
  ],
  'arpg-progression': [
