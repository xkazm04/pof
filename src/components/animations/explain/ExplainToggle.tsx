...
export function ExplainToggle({ label = 'Explain', compact = false }: ExplainToggleProps) {
  ...
  return (
    <button
      ...
      // Name stays stable; on/off is conveyed once, by `aria-checked`. Baking
      // the state into the name made screen readers announce it twice ("Explain
      // on …, switch, on").
      aria-label={`${label} — translate UE animation jargon into plain language`}
      ...
      className={[
        'focus-ring inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-2xs font-bold uppercase tracking-widest transition-colors border',
        enabled
    ...
    >
      ...
      {/* Visual echo of `aria-checked` — hidden from AT to avoid a duplicate
          state announcement. */}
      <span
        aria-hidden="true"
        className={[
          `inline-flex items-center justify-center min-w-[28px] px-1 py-[1px] rounded-full ${TEXT_SCALE.meta} font-mono`,
