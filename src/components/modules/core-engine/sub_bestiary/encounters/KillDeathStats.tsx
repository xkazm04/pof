...
import { KILL_DEATH_STATS, MAX_KILLS_ON_PLAYER, DEATH_CAUSES } from '../_shared/data';
import { STATUS_ERROR,
  withOpacity, OPACITY_20, OPACITY_10, OPACITY_25,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
          {KILL_DEATH_STATS.map((arch, i) => (
            <motion.div
              ...
              style={{ borderColor: `${withOpacity(arch.color, OPACITY_20)}` }}
            >
              <div className="flex items-center justify-between">
                ...
                <span
                  ...
                  style={{
                    backgroundColor: `${withOpacity(arch.color, OPACITY_10)}`,
                    borderColor: `${withOpacity(arch.color, OPACITY_25)}`,
                    color: arch.color,
                  }}
                >
                  #{arch.dangerRank}
    ...
    <div className="bg-surface-deep rounded-lg border border-border/30 p-3">
      ...
      <div className="flex items-center gap-3">
        <svg width={200} height={200} viewBox="0 0 64 64" className="flex-shrink-0">
          {DEATH_CAUSES.map((dc, i) => {
            const startAngle = DEATH_CAUSES.slice(0, i).reduce((s, d) => s + (d.pct / 100) * 360, 0);
