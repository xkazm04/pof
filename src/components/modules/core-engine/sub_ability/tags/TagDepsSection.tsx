...
import { MODULE_COLORS, ACCENT_RED, OVERLAY_WHITE, withOpacity, OPACITY_12, OPACITY_20, OPACITY_25, OPACITY_40 } from '@/lib/chart-colors';
import { BlueprintPanel, SectionHeader } from '../../unique-tabs/_design';
import { TAG_DEP_CATEGORIES } from '../_shared/data';
      ...
      <BlueprintPanel color={MODULE_COLORS.content} className="p-3">
          ...
          <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="overflow-visible">
            ...
            <defs>
              <marker id="arrowhead" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto">
                <polygon points="0 0, 8 3, 0 6" fill={withOpacity(OVERLAY_WHITE, OPACITY_40)} />
              </marker>
            </defs>
            ...
            {TAG_DEP_NODES.map((node, i) => {
              ...
              return (
                ...
                >
                  <circle cx={pos.x} cy={pos.y} r={16} fill={withOpacity(node.color, OPACITY_12)} stroke={node.color} strokeWidth={1.5} />
                  <text
                    x={pos.x} y={pos.y + 1}
                    textAnchor="middle" dominantBaseline="central"
                    className="text-xs font-mono font-bold" fill={node.color}
                  >
        ...
        <div className="flex items-center gap-4 justify-center mt-4 flex-wrap">
          {Object.entries(TAG_DEP_CATEGORIES).map(([cat, color]) => (
            <div key={cat} className="flex items-center gap-1.5 text-xs font-mono uppercase tracking-[0.15em]">
              <span className="w-3 h-3 rounded-full" style={{ backgroundColor: withOpacity(color, OPACITY_20), border: `1.5px solid ${color}` }} />
              <span style={{ color, textShadow: `0 0 12px ${withOpacity(color, OPACITY_25)}` }}>{cat}</span>
            </div>
          ))}
          <div className="flex items-center gap-1.5 text-xs font-mono uppercase tracking-[0.15em]">
