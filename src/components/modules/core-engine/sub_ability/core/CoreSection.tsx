...
import { MODULE_COLORS, OVERLAY_WHITE, withOpacity, OPACITY_5, OPACITY_8, OPACITY_15, OPACITY_25 } from '@/lib/chart-colors';
import {
  FeatureCard as SharedFeatureCard, PipelineFlow,
} from '../../unique-tabs/_shared';
import { BlueprintPanel, SectionHeader } from '../../unique-tabs/_design';
import { GAS_STEPS } from '../_shared/data';
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {['AttributeSet', 'Tag Container', 'Ability Instances', 'Active Effects'].map((conn, i) => (
            <motion.div
              ...
              style={{ borderColor: withOpacity(ACCENT, OPACITY_15), backgroundColor: withOpacity(ACCENT, OPACITY_5) }}
            >
              <span className="w-2 h-2 rounded-full" style={{ backgroundColor: ACCENT, boxShadow: `0 0 6px ${ACCENT}` }} />
              <span className="text-text font-medium" style={{ textShadow: `0 0 12px ${withOpacity(ACCENT, OPACITY_25)}` }}>{conn}</span>
            </motion.div>
          ))}
        </div>
      </BlueprintPanel>
        ...
        return (
          <g key={step.label}>
            <motion.rect
              ...
              rx={8} fill={withOpacity(step.color, OPACITY_8)}
            ...
            {!isLast && (
              ...
              >
                <line
                  ...
                  stroke={withOpacity(OVERLAY_WHITE, OPACITY_15)} strokeWidth={1.5}
                />
                <polygon
                  points={`${cx - 4},${y + stepH + arrowGap - 6} ${cx},${y + stepH + arrowGap} ${cx + 4},${y + stepH + arrowGap - 6}`}
                  fill={withOpacity(OVERLAY_WHITE, OPACITY_25)}
                />
              </motion.g>
            )}
          </g>
