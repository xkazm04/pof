/**
 * Shared base data + the three foundational GAS archetype templates
 * (melee combo, projectile spell, channeled ability).
 *
 * Extracted from templates.ts to break the templates <-> template-presets
 * import cycle: template-presets.ts imports these primitives from HERE, so it
 * no longer depends on templates.ts (which re-exports GAS_TEMPLATES back).
 * templates.ts stays the public barrel; consumers keep importing from it.
 */
