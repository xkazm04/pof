    ...
    <div className="space-y-2">
      ...
      <div className="relative overflow-x-auto custom-scrollbar">
        <svg ref={svgRef} width={svgW} height={svgH} viewBox={`0 0 ${svgW} ${svgH}`} className="overflow-visible">
          ...
          <defs><marker id="gas-arrow" viewBox="0 0 6 4" refX="6" refY="2" markerWidth="6" markerHeight="4" orient="auto"><path d="M0,0 L6,2 L0,4" fill={withOpacity(OVERLAY_WHITE, OPACITY_40)} /></marker></defs>
          {attributes.map((attr) => {
            ...
            return (
              <g key={attr.id} className="cursor-grab active:cursor-grabbing" onMouseDown={() => handleDragStart(attr.id)} onMouseUp={() => isDragTarget && handleDrop(attr.id)}>
                <rect x={pos.x - 2} y={pos.y} width={70} height={20} rx={4} fill={isDragTarget ? `${withOpacity(color, OPACITY_20)}` : `${withOpacity(color, OPACITY_10)}`} stroke={isDragTarget ? color : `${withOpacity(color, OPACITY_30)}`} strokeWidth={isDragTarget ? 1.5 : 0.8} />
