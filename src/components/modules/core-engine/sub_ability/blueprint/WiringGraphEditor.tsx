...
import {
  ...
  OVERLAY_WHITE,
  withOpacity, OPACITY_25, OPACITY_10, OPACITY_12, OPACITY_8, OPACITY_5, OPACITY_50, OPACITY_15,
  OPACITY_2, OPACITY_4, OPACITY_80, OPACITY_40, OPACITY_30, OPACITY_3,
...
export function WiringGraphEditor({
  ...
  // Adjacency indexes built once per nodes/wires change so the SVG render loops
  // below do O(1) lookups instead of repeated nodes.find()/wires.some() scans
  // (the previous code was O(wires·nodes) and re-ran on every hover).
  const { nodesById, wiresByNode, wireByEndpoint } = useMemo(() => {
    const byId = new Map<string, GASGraphNode>();
    for (const n of nodes) byId.set(n.id, n);
    const byNode = new Map<string, GraphWire[]>();
    const byEndpoint = new Map<string, GraphWire>();
    const add = (k: string, w: GraphWire) => { const arr = byNode.get(k); if (arr) arr.push(w); else byNode.set(k, [w]); };
    for (const w of wires) {
      add(w.fromNode, w);
      add(w.toNode, w);
      byEndpoint.set(`${w.fromNode}|${w.fromPin}`, w);
      byEndpoint.set(`${w.toNode}|${w.toPin}`, w);
    }
    return { nodesById: byId, wiresByNode: byNode, wireByEndpoint: byEndpoint };
  }, [nodes, wires]);
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between px-2 text-xs font-mono uppercase tracking-[0.2em] text-text-muted">
        ...
        <span style={{ color: STATUS_WARNING, textShadow: `0 0 12px ${withOpacity(STATUS_WARNING, OPACITY_25)}` }}>Effects</span>
        <span style={{ color: STATUS_ERROR, textShadow: `0 0 12px ${withOpacity(STATUS_ERROR, OPACITY_25)}` }}>Tag Rules</span>
      </div>
      <div className="relative overflow-x-auto custom-scrollbar rounded-lg border bg-surface-deep overflow-hidden" style={{ borderColor: `${withOpacity(ACCENT_VIOLET, OPACITY_10)}` }}>
        <svg ref={svgRef} width={svgW} height={maxY} viewBox={`0 0 ${svgW} ${maxY}`} className="overflow-visible" style={draggingNodeId ? { userSelect: 'none' } : undefined} onMouseMove={handleSvgMouseMove} onMouseUp={handleSvgMouseUp} onMouseLeave={handleSvgMouseUp}>
          <defs><marker id="gas-flow-arrow" viewBox="0 0 6 4" refX="6" refY="2" markerWidth="5" markerHeight="4" orient="auto"><path d="M0,0 L6,2 L0,4" fill={withOpacity(OVERLAY_WHITE, OPACITY_50)} /></marker></defs>
          <pattern id="gas-grid" width="20" height="20" patternUnits="userSpaceOnUse"><path d="M 20 0 L 0 0 0 20" fill="none" stroke={withOpacity(OVERLAY_WHITE, OPACITY_2)} strokeWidth="0.5" /></pattern>
          ...
          {nodes.map((node) => { const { x: nx, y: ny } = resolvePos(node); const isDragging = draggingNodeId === node.id; const isHovered = hoveredNode === node.id; const isSelected = selectedNode === node.id; const nodeWires = wiresByNode.get(node.id) ?? []; const hasConnections = nodeWires.length > 0; const sideCount = (side: 'left' | 'right') => node.pins.filter(p => p.side === side).length; const nodeH = Math.max(NODE_H_GRAPH, (Math.max(sideCount('left'), sideCount('right')) + 1) * 14 + 10); const dimmed = (hoveredNode || selectedNode) && !isHovered && !isSelected && !connectedWires.has(nodeWires[0]?.id ?? ''); const nodeIsConnected = nodeWires.some(w => connectedWires.has(w.id)); const effectiveOpacity = dimmed && !nodeIsConnected ? 0.25 : 1; return (<g key={node.id} className={isDragging ? 'cursor-grabbing' : 'cursor-grab'} onMouseEnter={() => { if (!draggingNodeId) setHoveredNode(node.id); }} onMouseLeave={() => { if (!draggingNodeId) setHoveredNode(null); }} onClick={() => { if (!dragMoved.current) setSelectedNode(prev => prev === node.id ? null : node.id); }} onMouseDown={(e) => { const target = e.target as SVGElement; if (target.tagName === 'circle') return; handleNodeDragStart(e, node.id); }} opacity={effectiveOpacity} style={{ transition: isDragging ? 'none' : 'opacity 0.2s' }}><rect x={nx} y={ny} width={NODE_W_GRAPH} height={nodeH} rx={6} fill={isDragging ? `${withOpacity(node.color, OPACITY_15)}` : isSelected ? `${withOpacity(node.color, OPACITY_12)}` : isHovered ? `${withOpacity(node.color, OPACITY_8)}` : `${withOpacity(node.color, OPACITY_5)}`} stroke={isDragging ? node.color : isSelected ? node.color : isHovered ? `${withOpacity(node.color, OPACITY_50)}` : `${withOpacity(node.color, OPACITY_25)}`} strokeWidth={isDragging ? 2 : isSelected ? 1.5 : 1} /><rect x={nx} y={ny} width={4} height={nodeH} rx={2} fill={node.color} opacity={isSelected || isDragging ? 0.8 : 0.5} /><text x={nx + 12} y={ny + 12} fill={isSelected || isHovered || isDragging ? node.color : withOpacity(OVERLAY_WHITE, OPACITY_80)} fontSize={node.type === 'tag-rule' ? 7 : 8.5} fontFamily="monospace" fontWeight="bold" className="pointer-events-none">{node.label.length > 18 ? node.label.slice(0, 17) + '\u2026' : node.label}</text><text x={nx + NODE_W_GRAPH - 4} y={ny + 11} fill={node.color} fontSize={6} fontFamily="monospace" textAnchor="end" opacity={0.5} className="pointer-events-none">{node.type === 'attribute' ? 'ATTR' : node.type === 'effect' ? 'GE' : 'TAG'}</text>{node.pins.map((pin) => { const pos = getPinPos(node, pin.id); const endpointWire = wireByEndpoint.get(`${node.id}|${pin.id}`); const isConnected = hasConnections && !!endpointWire; const wireColor = endpointWire?.color ?? node.color; return (<g key={pin.id}><circle cx={pos.x} cy={pos.y} r={PIN_R} fill={isConnected ? wireColor : withOpacity(OVERLAY_WHITE, OPACITY_10)} stroke={isConnected ? wireColor : withOpacity(OVERLAY_WHITE, OPACITY_30)} strokeWidth={1} className="pointer-events-auto" style={{ cursor: 'crosshair' }} onMouseDown={(e) => { e.stopPropagation(); setDragFromPin({ nodeId: node.id, pinId: pin.id }); }} onMouseUp={(e) => { e.stopPropagation(); setDragFromPin(null); }} />{pin.label && <text x={pin.side === 'left' ? pos.x + 6 : pos.x - 6} y={pos.y + 3} fill={withOpacity(OVERLAY_WHITE, OPACITY_40)} fontSize={6} fontFamily="monospace" textAnchor={pin.side === 'left' ? 'start' : 'end'} className="pointer-events-none">{pin.label.length > 12 ? pin.label.slice(0, 11) + '\u2026' : pin.label}</text>}</g>); })}{(isSelected || isDragging) && <rect x={nx - 2} y={ny - 2} width={NODE_W_GRAPH + 4} height={nodeH + 4} rx={8} fill="none" stroke={node.color} strokeWidth={1} opacity={0.3} style={{ filter: 'blur(3px)' }} className="pointer-events-none" />}</g>); })}
        </svg>
      </div>
      <div className="flex items-center gap-4 text-xs font-mono uppercase tracking-[0.15em] text-text-muted px-1">
        ...
        {nodeOverrides.size > 0 && <button onClick={() => setNodeOverrides(new Map())} className="ml-auto text-xs font-mono px-1.5 py-0.5 rounded border border-border/40 text-text-muted hover:text-text hover:border-border/60 transition-colors">Reset Layout</button>}
      </div>
      <AnimatePresence>
        {selectedDetail && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <div className="p-2.5 rounded-lg border space-y-1.5" style={{ borderColor: `${withOpacity(selectedDetail.node.color, OPACITY_15)}`, backgroundColor: `${withOpacity(selectedDetail.node.color, OPACITY_5)}` }}>
              <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full" style={{ backgroundColor: selectedDetail.node.color, boxShadow: `0 0 6px ${selectedDetail.node.color}` }} /><span className="text-xs font-mono font-bold" style={{ color: selectedDetail.node.color, textShadow: `0 0 12px ${withOpacity(selectedDetail.node.color, OPACITY_25)}` }}>{selectedDetail.node.label}</span><span className="text-xs font-mono uppercase tracking-[0.2em] text-text-muted">{selectedDetail.node.type}</span></div>
