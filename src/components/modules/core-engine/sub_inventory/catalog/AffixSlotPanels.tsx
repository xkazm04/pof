...
import { ACCENT, RARITY_COLORS, EQUIPMENT_SLOTS, AFFIX_EXAMPLES } from '../_shared/data';
      ...
      <BlueprintPanel color={ACCENT} className="p-0 overflow-hidden">
        <button onClick={() => setAffixOpen(v => !v)}
          ...
          <span className="text-xs font-mono uppercase tracking-[0.15em] px-2 py-0.5 rounded-md border shadow-sm ml-auto"
            style={{ backgroundColor: STATUS_COLORS[featureMap.get('Affix system')?.status ?? 'unknown'].bg, color: STATUS_COLORS[featureMap.get('Affix system')?.status ?? 'unknown'].dot, borderColor: `${withOpacity(STATUS_COLORS[featureMap.get('Affix system')?.status ?? 'unknown'].dot, OPACITY_25)}` }}>
                      ...
                      <motion.tr key={affix.name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                        ...
                        <td className="px-4 py-2">
                          <span className="px-2 py-0.5 rounded-md font-mono text-sm uppercase font-bold border shadow-sm"
                            style={{ backgroundColor: withOpacity(affix.tier === 'Prefix' ? MODULE_COLORS.core : MODULE_COLORS.systems, OPACITY_10), color: affix.tier === 'Prefix' ? MODULE_COLORS.core : MODULE_COLORS.systems, borderColor: withOpacity(affix.tier === 'Prefix' ? MODULE_COLORS.core : MODULE_COLORS.systems, OPACITY_25) }}>
                            {affix.tier}
                          </span>
                        </td>
          ...
          {EQUIPMENT_SLOTS.map(slot => {
            ...
            return (
              <div key={slot.id} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm shadow-sm border"
                style={{ backgroundColor: withOpacity(sc.dot, OPACITY_10), borderColor: withOpacity(sc.dot, OPACITY_20) }}>
