  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-mono uppercase tracking-[0.15em] w-16 truncate" style={{ color: config.color }}>
          {config.label}
        </span>
        <div className="flex-1 relative h-4 flex items-center">
          ...
          />
          <motion.div
            animate={focused ? { scale: 1.4 } : { scale: 1 }}
      ...
      <div className="flex gap-1 ml-8 mt-0.5 flex-wrap">
        {gene.affinityTags.map((tag) => (
          <span
            key={tag}
            className="text-xs font-mono uppercase tracking-[0.15em] px-1 py-0 rounded"
