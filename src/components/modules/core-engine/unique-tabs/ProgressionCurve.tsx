...
import {
  ...
  Code, Copy, CheckCircle2, Download, Table2,
} from 'lucide-react';
...
/* ── 8.11 Diminishing Returns C++ Code Generator ───────────────────────────── */
...
/* ── 8.12 Data-Driven XP Curve Table Generator ─────────────────────────────── */
...
function generateProgressionTable(
  ...
  for (let lvl = 1; lvl <= maxLevel; lvl++) {
    ...
    xpTotal += xpRequired;
    const hpBonus = override?.hpBonus ?? hpPerLevel;
    const manaBonus = override?.manaBonus ?? manaPerLevel;
...
export function ProgressionCurve({ moduleId }: ProgressionCurveProps) {
  ...
  // 8.12 XP Data Table generator state
  ...
  return (
    <div className="space-y-2.5">
      ...
      {/* ═══════════════════════════════════════════════════════════════════════ */}
      {/*  8.11  DIMINISHING RETURNS C++ IMPLEMENTATION GENERATOR               */}
      {/* ═══════════════════════════════════════════════════════════════════════ */}
      <SurfaceCard level={2} className="p-5 relative overflow-hidden">
        <SectionLabel icon={Code} label="Diminishing Returns C++ Generator" color={ACCENT} />

        {/* Config editors per attribute */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5 mt-2.5 mb-3">
        ...
        <div className="mt-3">
          ...
          <div className="w-full h-[160px] bg-surface-deep/50 rounded-xl relative p-4 border border-border/40">
            <svg className="w-full h-full overflow-visible" viewBox="0 0 100 100" preserveAspectRatio="none">
              {drConfigs.map((config, ci) => {
                const drAttr = DR_ATTRIBUTES.find(a => a.name === config.attribute);
                const color = drAttr?.color ?? ACCENT;
