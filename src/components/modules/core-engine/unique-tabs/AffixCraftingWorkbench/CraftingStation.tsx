        ...
        <button onClick={onToggleCraftPanel} className="flex items-center gap-2">
          ...
          {prefixLocked && (
            <span className="text-xs px-1.5 py-0.5 rounded font-bold font-mono"
              style={{ backgroundColor: `${ACCENT_PURPLE}${OPACITY_15}`, color: ACCENT_PURPLE, border: `1px solid ${withOpacity(ACCENT_PURPLE, OPACITY_20)}` }}>
              PRE LOCKED
            </span>
          )}
          {suffixLocked && (
        ...
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-3">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            {CURRENCIES.map(c => (
              <div key={c.id}
                ...
                style={{
                  border: `1px solid ${wallet[c.id] === 0 ? withOpacity(STATUS_ERROR, OPACITY_25) : withOpacity(c.color, OPACITY_12)}`,
                  backgroundColor: wallet[c.id] === 0 ? withOpacity(STATUS_ERROR, OPACITY_5) : withOpacity(c.color, OPACITY_5),
                }}
                title={`${c.name}: ${c.description}`}>
                <span className="text-xs">{c.icon}</span>
                <span className="text-xs font-mono font-bold"
                  style={{ color: wallet[c.id] === 0 ? STATUS_ERROR : c.color, textShadow: `0 0 12px ${withOpacity(c.color, OPACITY_25)}` }}>
                  {wallet[c.id]}
                </span>
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={onResetWallet}
              ...
              style={{ border: `1px solid ${withOpacity(ACCENT_PURPLE, OPACITY_15)}` }}>
            ...
            {CRAFTING_ACTIONS.map(action => {
              ...
              return (
                <motion.button key={action.id}
                  ...
                  style={{
                    border: `1px solid ${isFlashing ? withOpacity(action.color, OPACITY_50) : enabled ? withOpacity(action.color, OPACITY_20) : withOpacity(action.color, OPACITY_10)}`,
                    backgroundColor: isFlashing ? withOpacity(action.color, OPACITY_12) : enabled ? withOpacity(action.color, OPACITY_5) : 'transparent',
                    boxShadow: isFlashing ? `0 0 12px ${withOpacity(action.color, OPACITY_20)}` : 'none',
                  }}
                  ...
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-xs font-mono font-bold uppercase tracking-[0.15em]"
                      style={{ color: enabled ? action.color : 'var(--text-muted)', textShadow: enabled ? `0 0 12px ${withOpacity(action.color, OPACITY_25)}` : 'none' }}>
                      {action.name}
                    </span>
                  </div>
                    ...
                    {Object.entries(action.costs).map(([cid, amount]) => {
                      ...
                      return (
                        <span key={cid} className="text-xs font-mono px-1 py-0.5 rounded"
                          style={{ backgroundColor: hasEnough ? withOpacity(cur.color, OPACITY_8) : withOpacity(STATUS_ERROR, OPACITY_8), color: hasEnough ? cur.color : STATUS_ERROR }}>
                          {amount} {cur.icon}
                        </span>
                      );
                    })}
