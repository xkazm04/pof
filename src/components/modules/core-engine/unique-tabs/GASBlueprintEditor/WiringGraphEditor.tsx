        ...
        {selectedDetail && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <div className="p-2.5 rounded-lg border space-y-1.5" style={{ borderColor: `${selectedDetail.node.color}25`, backgroundColor: `${selectedDetail.node.color}08` }}>
              <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full" style={{ backgroundColor: selectedDetail.node.color, boxShadow: `0 0 6px ${selectedDetail.node.color}` }} /><span className="text-xs font-mono font-bold" style={{ color: selectedDetail.node.color, textShadow: `0 0 12px ${selectedDetail.node.color}40` }}>{selectedDetail.node.label}</span><span className="text-xs font-mono uppercase tracking-[0.2em] text-text-muted">{selectedDetail.node.type}</span></div>
