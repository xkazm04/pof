  ...
  return (
    <div className="space-y-2">
      <div className="relative overflow-x-auto custom-scrollbar">
        ...
        {playheadTime !== null && activeEffectDetails.length > 0 && (
          <div className="absolute top-0 right-0 mt-1 mr-1 p-1.5 rounded-lg border border-border/40 bg-surface-deep/90 backdrop-blur-sm max-w-[160px]">
            <div className="text-[10px] font-mono font-bold text-text-muted uppercase tracking-[0.15em] mb-1">Active @ {playheadTime.toFixed(1)}s</div>
            {activeEffectDetails.map((eff) => (<div key={eff.id} className="flex items-center gap-1.5 mb-0.5"><span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: eff.color, boxShadow: `0 0 4px ${eff.color}` }} /><span className="text-[10px] font-mono font-bold truncate" style={{ color: eff.color, textShadow: `0 0 12px ${eff.color}40` }}>{eff.name}</span></div>))}
            {activeEffectDetails.some(e => e.grantedTags.length > 0) && (
              <div className="mt-1 pt-1 border-t border-border/30"><div className="text-[10px] font-mono text-text-muted uppercase tracking-[0.15em] mb-0.5">Tags</div>
                <div className="flex flex-wrap gap-0.5">{activeEffectDetails.flatMap(e => e.grantedTags.map(t => ({ tag: t, color: e.color }))).map(({ tag, color }) => (<span key={tag} className="text-[10px] font-mono px-1 py-0 rounded" style={{ backgroundColor: `${color}08`, color, border: `1px solid ${color}25` }}>{tag}</span>))}</div>
              </div>
            )}
          </div>
        )}
      ...
      <div className="flex items-center gap-2">
        <button onClick={addEffect} className="flex items-center gap-1.5 px-2.5 py-1 rounded text-[10px] font-mono uppercase tracking-[0.15em] font-medium" style={{ backgroundColor: `${ACCENT}15`, color: ACCENT, border: `1px solid ${ACCENT}30` }}><Plus className="w-3 h-3" /> Add Effect</button>
      </div>
      <AnimatePresence>
        {sel && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <div className="p-2.5 rounded-lg border space-y-3" style={{ borderColor: `${sel.color}25`, backgroundColor: `${sel.color}08` }}>
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1"><label className="block text-[10px] text-text-muted font-bold uppercase tracking-[0.15em] mb-1">Duration Type</label><SegmentedControl options={DURATION_OPTIONS} activeId={sel.duration} onChange={(id) => updateEffect(sel.id, { duration: id as EffectDuration })} accent={sel.color} /></div>
                {sel.duration === 'duration' && <div><label className="block text-[10px] text-text-muted font-bold uppercase tracking-[0.15em] mb-1">Seconds</label><input type="number" value={sel.durationSec} min={0} step={0.5} onChange={(e) => updateEffect(sel.id, { durationSec: Number(e.target.value) })} className="w-16 bg-surface-deep border border-border/40 rounded px-1.5 py-0.5 text-xs font-mono text-text focus:outline-none focus:border-current" style={{ color: sel.color }} /></div>}
                <div><label className="block text-[10px] text-text-muted font-bold uppercase tracking-[0.15em] mb-1">Cooldown</label><input type="number" value={sel.cooldownSec} min={0} step={0.5} onChange={(e) => updateEffect(sel.id, { cooldownSec: Number(e.target.value) })} className="w-16 bg-surface-deep border border-border/40 rounded px-1.5 py-0.5 text-xs font-mono text-text focus:outline-none focus:border-current" /></div>
              </div>
              <div className="space-y-1.5"><div className="flex items-center justify-between"><label className="text-[10px] text-text-muted font-bold uppercase tracking-[0.15em]">Modifiers</label><button onClick={() => updateEffect(sel.id, { modifiers: [...sel.modifiers, { attribute: 'Health', operation: 'add', magnitude: 0 }] })} className="text-2xs text-text-muted hover:text-text"><Plus className="w-3 h-3" /></button></div>
