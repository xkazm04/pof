            ...
            {templates.map((tpl, i) => {
              ...
              return (
                <motion.button key={tpl.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  style={{ borderColor: isActive ? tpl.color : `${withOpacity(tpl.color, OPACITY_15)}`, backgroundColor: isActive ? `${withOpacity(tpl.color, OPACITY_8)}` : 'transparent' }}
                >
                  <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
                    style={{ background: `radial-gradient(ellipse at 50% 0%, ${withOpacity(tpl.color, OPACITY_8)} 0%, transparent 70%)` }} />
                  {isActive && (
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {tpl.tags.map((tag) => (
                        <span key={tag} className="text-xs font-mono px-1.5 py-0 rounded"
                          style={{ backgroundColor: `${withOpacity(tpl.color, OPACITY_8)}`, color: `${withOpacity(tpl.color, OPACITY_80)}`, border: `1px solid ${withOpacity(tpl.color, OPACITY_12)}` }}>
                          {tag}
                        </span>
                      ))}
                    </div>
                    <div className="mt-2.5 rounded overflow-hidden" style={{ backgroundColor: `${withOpacity(tpl.color, OPACITY_5)}` }}>
                      <svg width="100%" height={36} viewBox="0 0 280 36" preserveAspectRatio="xMinYMin" className="overflow-visible">
                        {tpl.attributes.filter(a => a.category === 'combat' || a.category === 'vital').slice(0, 4).map((attr, ai) => (
                          <g key={attr.id}>
                            <circle cx={20 + ai * 18} cy={12} r={3.5} fill={`${withOpacity(tpl.color, OPACITY_25)}`} stroke={`${withOpacity(tpl.color, OPACITY_50)}`} strokeWidth={0.5} />
                            <line x1={20 + ai * 18} y1={16} x2={120 + (ai % tpl.effects.length) * 22} y2={20} stroke={`${withOpacity(tpl.color, OPACITY_15)}`} strokeWidth={0.5} />
                          </g>
                        ))}
                        {tpl.effects.slice(0, 5).map((eff, ei) => (
                          <g key={eff.id}>
                            <rect x={108 + ei * 22} y={18} width={18} height={10} rx={2} fill={`${withOpacity(eff.color, OPACITY_20)}`} stroke={`${withOpacity(eff.color, OPACITY_37)}`} strokeWidth={0.5} />
                            {ei < tpl.tagRules.length - 2 && <line x1={126 + ei * 22} y1={23} x2={230 + (ei % 3) * 16} y2={12} stroke={`${withOpacity(tpl.color, OPACITY_12)}`} strokeWidth={0.5} />}
                          </g>
                        ))}
                        {tpl.tagRules.slice(0, 4).map((_, ti) => (
                          <circle key={ti} cx={235 + ti * 14} cy={12} r={3} fill={`${withOpacity(STATUS_ERROR, OPACITY_20)}`} stroke={`${withOpacity(STATUS_ERROR, OPACITY_37)}`} strokeWidth={0.5} />
                        ))}
                      </svg>
                    </div>
                  </div>
