...
import { MODULE_COLORS, ACCENT_CYAN, STATUS_SUCCESS } from '@/lib/chart-colors';
import type { FeatureRow, FeatureStatus } from '@/types/feature-matrix';
import { BlueprintPanel, SectionHeader } from '../_design';
import { STATUS_COLORS, PipelineFlow } from '../_shared';
import { AI_PIPELINE, BT_NODES, BT_EDGES, DETECTED_ENTITIES } from './data';
        ...
        <BlueprintPanel color={accent} className="p-0 overflow-hidden h-fit">
          <button onClick={() => setSpawnOpen(!spawnOpen)}
            ...
            <span className="text-xs font-bold uppercase px-2 py-0.5 rounded-md ml-auto border shadow-sm"
              style={{ backgroundColor: spawnSc.bg, color: spawnSc.dot, borderColor: `${spawnSc.dot}40` }}>
              {spawnSc.label}
            </span>
          </button>
          <AnimatePresence>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <BlueprintPanel color={STATUS_SUCCESS} className="p-3">
          ...
          <div className="mt-3 flex flex-col md:flex-row gap-4">
            <BTFlowchart nodes={BT_NODES} edges={BT_EDGES}
...
function BtDetailsPanel({ expandedNodeId }: { expandedNodeId: string | null }) {
  const node = expandedNodeId ? BT_NODES.find(n => n.id === expandedNodeId) : null;
  return (
          ...
          <motion.div key={expandedNodeId}
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={`text-xs font-bold px-1.5 py-0.5 rounded ${
                node.active ? 'bg-emerald-500/20 text-emerald-400' : 'bg-gray-500/20 text-gray-400'
              }`}>
