  ...
  return (
    <div>
      ...
      <input
        ...
        style={{
          background: `linear-gradient(to right, ${color} 0%, ${color} ${((value - min) / (max - min)) * 100}%, ${withOpacity(OVERLAY_WHITE, OPACITY_10)} ${((value - min) / (max - min)) * 100}%, ${withOpacity(OVERLAY_WHITE, OPACITY_10)} 100%)`,
        }}
      />
    </div>
  );
