...
import { OPACITY_20, STATUS_SUCCESS, STATUS_WARNING, ACCENT_CYAN,
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 text-2xs font-mono text-text-muted">
            ...
            <input type="number" min={10} max={10000} step={10} value={simKillCount}
              ...
              className="w-16 bg-surface-deep/50 border border-border/40 rounded px-1.5 py-0.5 text-2xs font-mono text-text text-right focus:outline-none focus:ring-1 focus:ring-current/50" />
          </div>
          <button onClick={() => setShowLootCpp(!showLootCpp)}
            ...
            style={{ borderColor: withOpacity(ACCENT, OPACITY_20), color: ACCENT, backgroundColor: withOpacity(ACCENT, OPACITY_5) }}>
      ...
      <AnimatePresence>
        {showLootCpp && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mt-3">
            <div className="bg-surface-deep rounded-xl border border-border/40 overflow-hidden">
              <div className="px-3 py-1.5 bg-surface-deep/50 border-b border-border/30 flex items-center justify-between">
                ...
                <button onClick={handleCopyLootCpp} className="flex items-center gap-1 px-2 py-1 rounded text-xs font-mono font-bold"
                  style={{ color: copiedLootCpp ? STATUS_SUCCESS : ACCENT, backgroundColor: copiedLootCpp ? withOpacity(STATUS_SUCCESS, OPACITY_10) : withOpacity(ACCENT, OPACITY_8) }}>
                  {copiedLootCpp ? <CheckCircle2 className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                  {copiedLootCpp ? 'Copied' : 'Copy'}
                </button>
              </div>
              <pre className="p-4 text-2xs font-mono leading-relaxed overflow-x-auto custom-scrollbar max-h-[350px] overflow-y-auto" style={{ color: withOpacity(ACCENT_CYAN, OPACITY_90) }}>
