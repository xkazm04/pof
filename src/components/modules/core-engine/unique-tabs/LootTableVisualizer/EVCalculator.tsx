...
import { OPACITY_20, STATUS_WARNING } from '@/lib/chart-colors';
...
export function EVCalculator() {
  ...
  return (
    <BlueprintPanel className="p-4 relative overflow-hidden">
      ...
      <div className="mb-4">
        <div className="text-[10px] font-mono uppercase tracking-[0.15em] text-text-muted mb-1.5">Item Sell Values (gold per rarity)</div>
        <div className="flex flex-wrap gap-2">
          {RARITY_TIERS.map(tier => (
            <div key={tier.name} className="flex items-center gap-1.5 rounded-lg border px-2 py-1" style={{ borderColor: `${tier.color}${OPACITY_20}`, backgroundColor: `${tier.color}08` }}>
              ...
              <input type="number" min={0} step={5} value={rarityGold[tier.name]}
                ...
                className="w-16 bg-surface-deep/50 border border-border/40 rounded px-1.5 py-0.5 text-[11px] font-mono text-text text-right focus:outline-none focus:ring-1 focus:ring-orange-500/50" />
              <span className="text-[11px] font-mono text-text-muted">g</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Session parameters */}
      <div className="flex flex-wrap items-center gap-4 mb-4 text-[11px] font-mono text-text-muted">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
        {evResults.map(r => (
          <motion.div key={r.archetypeId} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            className="rounded-lg border p-3" style={{ borderColor: `${r.color}${OPACITY_20}`, backgroundColor: `${r.color}05` }}>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex justify-between text-[11px] font-mono pl-3 text-text-muted">
      ...
      <div className="mb-4">
        <div className="text-[10px] font-mono uppercase tracking-[0.15em] text-text-muted mb-2">Farming Efficiency (gold/hour)</div>
        <div className="space-y-2">
          {evResults.map((r, i) => (
            <div key={r.archetypeId} className="flex items-center gap-2">
              <span className="text-[11px] font-mono w-24 text-right truncate" style={{ color: r.color }}>{r.archetypeName}</span>
              <div className="flex-1 h-5 bg-surface-deep/50 rounded-full overflow-hidden relative">
                <motion.div className="h-full rounded-full flex items-center justify-end pr-2" style={{ backgroundColor: r.color }}
                  ...
                  <span className="text-[10px] font-mono font-bold text-black/80 whitespace-nowrap">{Math.round(r.evPerHour).toLocaleString()}g/hr</span>
                </motion.div>
              </div>
            </div>
          ))}
      ...
      <div className="rounded-lg border p-3" style={{ borderColor: `${ACCENT}${OPACITY_20}`, backgroundColor: `${ACCENT}05` }}>
        <div className="flex items-center gap-2 mb-2">
          <Target className="w-3.5 h-3.5" style={{ color: ACCENT }} />
          <span className="text-[11px] font-mono font-bold" style={{ color: ACCENT }}>Breakeven Calculator</span>
        </div>
        <label className="flex items-center gap-1.5 text-[11px] font-mono text-text-muted mb-3">
          Target gold:
          <input type="number" min={100} step={1000} value={evTargetGold}
            onChange={e => setEvTargetGold(Math.max(100, Number(e.target.value)))}
            className="w-24 bg-surface-deep/50 border border-border/40 rounded px-1.5 py-0.5 text-[11px] font-mono text-text text-right focus:outline-none focus:ring-1 focus:ring-orange-500/50" />
          <span>g</span>
        </label>
        <div className="space-y-1.5">
          {evResults.map(r => {
            ...
            return (
              <div key={r.archetypeId} className="flex items-center gap-3 text-[11px] font-mono">
