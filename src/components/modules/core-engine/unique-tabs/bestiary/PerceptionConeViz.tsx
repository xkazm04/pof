...
import { ACCENT_CYAN, OPACITY_4, OPACITY_12, OPACITY_25, OPACITY_30, OPACITY_50, OVERLAY_WHITE, withOpacity, GLOW_SM } from '@/lib/chart-colors';
...
export function PerceptionConeViz({ entities, accent = ACCENT_CYAN }: PerceptionConeVizProps) {
  return (
    <svg width={200} height={200} viewBox="0 0 130 130" className="flex-shrink-0">
      ...
      {entities.map(e => (
        <g key={e.label}>
          <circle cx={e.x} cy={e.y} r={4} fill={e.color} style={{ filter: `drop-shadow(${GLOW_SM} ${e.color})` }} />
