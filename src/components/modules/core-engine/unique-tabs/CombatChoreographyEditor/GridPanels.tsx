...
export function ArchetypePalette({ selectedArchetype, onSelectArchetype, placeLevel, onPlaceLevel, playerLevel, onPlayerLevel }: {
  ...
  return (
    <div className="space-y-1.5 shrink-0">
      <div className="text-[10px] font-mono font-bold uppercase tracking-[0.15em] text-text-muted">Archetype</div>
      {ENEMY_ARCHETYPES.map((arch) => {
        ...
        return (
          <button key={arch.id} onClick={() => onSelectArchetype(arch.id)}
            ...
            <span className="w-4 h-4 rounded-sm flex items-center justify-center text-[11px]"
      ...
      <div className="text-[10px] font-mono font-bold uppercase tracking-[0.15em] text-text-muted">Level</div>
      ...
      <div className="text-[10px] font-mono font-bold uppercase tracking-[0.15em] text-text-muted">Player Lv</div>
      <div className="flex items-center gap-1">
...
export function GhostLegend({ selectedWave, totalWaves }: { selectedWave: number; totalWaves: number }) {
  return (
    <div className="flex items-center gap-3 text-[10px] font-mono uppercase tracking-[0.15em] text-text-muted/60 pt-1 border-t border-border/20 flex-wrap">
