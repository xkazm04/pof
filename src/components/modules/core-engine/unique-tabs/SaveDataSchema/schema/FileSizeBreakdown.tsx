    ...
    <BlueprintPanel color={ACCENT} className="p-0 overflow-hidden">
      ...
      <div className="p-4 space-y-4 relative z-10">
          ...
          {FILE_SIZE_SECTIONS.map((sec) => {
            ...
            return (
              <motion.div
                ...
                style={{ backgroundColor: `${withOpacity(sec.color, OPACITY_20)}`, borderLeft: `2px solid ${sec.color}` }}
                title={`${sec.label}: ${formatBytes(sec.bytes)} (${pct.toFixed(1)}%)`}
              >
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity" style={{ backgroundColor: `${withOpacity(sec.color, OPACITY_10)}` }} />
                {pct > 12 && (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-2">
          {FILE_SIZE_SECTIONS.map((sec) => (
            <div key={sec.label} className="border border-border/10 rounded-lg p-2 font-mono text-xs" style={{ backgroundColor: `${withOpacity(ACCENT, OPACITY_5)}` }}>
              <div className="flex items-center gap-1.5 mb-1">
                <span className="w-2 h-2 rounded-sm" style={{ backgroundColor: sec.color }} />
                <span className="font-bold truncate text-xs uppercase tracking-[0.15em]" style={{ color: ACCENT_CYAN_LIGHT }}>{sec.label}</span>
              </div>
              <div style={{ color: OVERLAY_WHITE }}>{formatBytes(sec.bytes)}</div>
        ...
        <div className="flex items-center gap-3 px-2 py-2 border border-border/10 rounded-lg font-mono text-xs" style={{ backgroundColor: `${withOpacity(ACCENT, OPACITY_5)}` }}>
          ...
          <span style={{ color: ACCENT_CYAN_LIGHT }}>{formatBytes(Math.round(TOTAL_BYTES * COMPRESSION_RATIO))}</span>
          <span className="font-bold" style={{ color: STATUS_SUCCESS }}>{((1 - COMPRESSION_RATIO) * 100).toFixed(0)}% saved</span>
        </div>
      </div>
    </BlueprintPanel>
  );
