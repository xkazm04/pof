        ...
        <div className="p-4 space-y-3 relative z-10 flex-1 overflow-y-auto">
          {SAVE_SLOTS.map((slot, i) => (
            <motion.div
              ...
              style={!slot.empty ? { backgroundColor: slot.isAuto ? withOpacity(STATUS_WARNING, OPACITY_5) : withOpacity(ACCENT, OPACITY_5) } : undefined}
            >
              {slot.empty ? (
                ...
                <>
                  <div className={`absolute left-0 top-0 bottom-0 w-1 ${slot.isAuto ? '' : 'opacity-50 group-hover:opacity-100 transition-opacity'}`} style={{ backgroundColor: slot.isAuto ? STATUS_WARNING : ACCENT_CYAN }} />

                  <div className="flex justify-between items-start mb-2 pl-2">
                    <span className="text-xs font-bold tracking-[0.15em] uppercase" style={{ color: slot.isAuto ? STATUS_WARNING : ACCENT_CYAN_LIGHT }}>{slot.label}</span>
