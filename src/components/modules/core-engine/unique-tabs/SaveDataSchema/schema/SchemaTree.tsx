    ...
    <BlueprintPanel color={ACCENT} className="p-0 flex flex-col h-full overflow-hidden">
      <div className="px-4 py-3 border-b border-border/10 flex items-center justify-between">
        ...
        <span className="flex gap-1.5 items-center">
          <span className="w-2 h-2 rounded-full" style={{ backgroundColor: withOpacity(STATUS_ERROR, OPACITY_50) }} />
          <span className="w-2 h-2 rounded-full" style={{ backgroundColor: withOpacity(STATUS_WARNING, OPACITY_50) }} />
          <span className="w-2 h-2 rounded-full" style={{ backgroundColor: withOpacity(STATUS_SUCCESS, OPACITY_50) }} />
        </span>
      </div>

      <div className="flex flex-wrap items-center gap-3 px-4 py-1.5 border-b border-border/10 text-xs font-mono" style={{ backgroundColor: `${withOpacity(ACCENT, OPACITY_5)}` }}>
      ...
      <div className="p-4 space-y-1 font-mono text-xs leading-relaxed overflow-y-auto custom-scrollbar relative">
        <div className="absolute left-0 top-0 bottom-0 w-8 border-r border-border/10 z-0 select-none flex flex-col pt-4 items-center text-xs font-mono" style={{ backgroundColor: `${withOpacity(ACCENT, OPACITY_5)}`, color: `${withOpacity(ACCENT, OPACITY_15)}` }}>
        ...
        <div className="relative z-10 pl-6" style={{ color: `${withOpacity(ACCENT, OPACITY_80)}` }} ref={treeRef} role="tree" aria-label="Save data schema tree" onKeyDown={handleTreeKeyDown}>
              ...
              return (
                <motion.div key={group.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 * gi }} className="mb-1">
                  ...
                  >
                    ...
                    <span style={{ color: group.color, textShadow: `0 0 5px ${withOpacity(group.color, OPACITY_25)}` }}>{group.label}</span>
                  </button>
                  <AnimatePresence>
                    {isOpen && (
                      <motion.div role="group" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden pl-6">
                        {group.fields.map((field) => (
                          <div key={field.name} role="treeitem" tabIndex={-1} aria-label={`${field.name}, type ${field.type}`}
                            ...
                            <span style={{ color: OVERLAY_WHITE }}>{field.name};</span>
          ...
          <div className="mt-1" style={{ color: `${withOpacity(ACCENT, OPACITY_80)}` }}>{'};'}</div>
        </div>
        <div className="pl-6 mt-2.5" style={{ color: `${withOpacity(ACCENT, OPACITY_25)}` }}>&gt; EOF</div>
      </div>
    </BlueprintPanel>
  );
}
