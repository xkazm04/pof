  ...
  return (
    <div className="flex flex-wrap gap-2 mb-2">
      {[phases.movement, phases.invuln, phases.cancel, phases.recovery].map((phase) => (
        <span key={phase.label} className="flex items-center gap-1 text-[10px] font-mono uppercase tracking-[0.15em]" style={{ color: phase.color }}>
          <span className="w-3 h-1 rounded-full" style={{ backgroundColor: phase.color }} />
          {phase.label}
        </span>
      ))}
      {hitMarkers.length > 0 && (
        <span className="flex items-center gap-1 text-[10px] font-mono uppercase tracking-[0.15em]" style={{ color: STATUS_ERROR }}>
    ...
    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
      <BlueprintPanel color={STATUS_WARNING} className="p-3">
        <div className="flex items-center gap-2 mb-2.5">
          ...
          <span className="ml-auto text-[10px] font-mono uppercase tracking-[0.15em] text-text-muted/50">@60 FPS &middot; Dustloop-style</span>
        </div>
