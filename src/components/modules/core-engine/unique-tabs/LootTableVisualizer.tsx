      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {WORLD_ITEMS.map((item) => (
            <SurfaceCard key={item.name} level={2} className="px-3 py-2 flex items-start gap-2">
              <div
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  <Package className="w-3 h-3 flex-shrink-0" style={{ color: item.beamColor }} />
                  <span className="text-xs font-medium text-text truncate">{item.name}</span>
                </div>
                <span
                  className="text-2xs font-medium"
                  style={{ color: item.beamColor }}
                >
                  {item.rarity}
                </span>
                <p className="text-2xs text-text-muted mt-0.5">{item.pickup}</p>
              </div>
            </SurfaceCard>
          ))}
        </div>
