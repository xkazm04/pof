            ...
            return (
              <motion.div key={b.label} initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }}>
                <div className="flex items-center gap-3 mb-1">
                  <span className="text-[10px] font-mono uppercase tracking-[0.15em] font-bold text-text w-36 truncate">{b.label}</span>
                  <span className="text-[10px] font-mono uppercase tracking-[0.15em] text-text-muted">
                    {b.current}{b.unit ? ` ${b.unit}` : ''} / {b.max}{b.unit ? ` ${b.unit}` : ''}
                  </span>
                  <span className="ml-auto text-[10px] font-mono uppercase tracking-[0.15em] font-bold" style={{ color: barColor }}>
                    {Math.round(pct * 100)}%
                  </span>
                </div>
              ...
              <motion.div key={layer.depth} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="relative">
                ...
                <div className="flex items-start gap-3">
                  <div className="w-12 pt-0.5 text-right flex-shrink-0">
                    <span className="text-[10px] font-mono uppercase tracking-[0.15em] text-text-muted">Z: {layer.depth}</span>
                  </div>
                  <div className="flex-1 bg-surface/50 rounded-md border border-border/30 p-2 relative overflow-hidden">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {layer.widgets.map(w => (
                          <span key={w} className="px-1.5 py-0.5 bg-background rounded border border-border/50 text-[11px] font-mono text-text-muted">{w}</span>
                        ))}
                      </div>
                    </div>
                    {layer.hasOverlap && (
                      <div className="absolute top-1.5 right-1.5 text-amber-500"><AlertCircle className="w-3 h-3" /></div>
                    )}
                  </div>
                </div>
              </motion.div>
