            ...
            return (
              <g key={`edge-${i}`}>
                <line x1={sx} y1={sy} x2={tx} y2={ty}
                  stroke={isHighlighted ? ACCENT : withOpacity(OVERLAY_WHITE, OPACITY_12)}
                  strokeWidth={isHighlighted ? 2 : 1.5}
                  strokeDasharray={edge.style === 'dashed' ? '6 4' : undefined}
                  style={{ transition: 'stroke 0.2s, stroke-width 0.2s' }} />
                <motion.line
                  x1={sx} y1={sy} x2={tx} y2={ty}
                  stroke={isHighlighted ? ACCENT : withOpacity(OVERLAY_WHITE, OPACITY_20)}
            ...
            return (
              <g key={node.id}
                ...
                <circle cx={x} cy={y} r={isHighlighted ? 28 : 24}
                  ...
                  style={{ filter: isHighlighted ? `drop-shadow(0 0 10px ${nodeColor})` : `drop-shadow(${GLOW_SM} ${withOpacity(nodeColor, OPACITY_25)})`, transition: 'all 0.2s' }} />
      ...
      <div className="flex flex-wrap gap-3 mt-3 justify-center">
        {Object.entries(FLOW_GROUP_COLORS).map(([group, color]) => (
          <span key={group} className="flex items-center gap-1.5 text-xs font-mono uppercase tracking-[0.15em] font-bold" style={{ color }}>
            <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: color, boxShadow: `${GLOW_SM} ${color}` }} />
            {group}
          </span>
        ))}
      </div>
