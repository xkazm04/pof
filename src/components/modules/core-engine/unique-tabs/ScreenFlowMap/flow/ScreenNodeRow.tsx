...
export function InputModeBadge({ mode }: { mode: InputMode }) {
  ...
  return (
    <span
      ...
      style={{ backgroundColor: `${withOpacity(color, OPACITY_10)}`, color, borderColor: `${withOpacity(color, OPACITY_25)}` }}
    >
      {mode}
    </span>
  );
          ...
          <div className="flex sm:hidden mt-2 ml-6 items-center gap-2">
            ...
            {arrowLabel && (
              <span className="text-xs font-mono uppercase tracking-[0.15em] px-1.5 py-0.5 rounded font-medium border"
                style={{ backgroundColor: `${withOpacity(ACCENT, OPACITY_8)}`, color: ACCENT, borderColor: `${withOpacity(ACCENT, OPACITY_20)}` }}>
                Trigger: {arrowLabel}
              </span>
            )}
          </div>
          ...
          {isExpanded && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
              <div className="px-4 pb-3 pt-1 border-t border-border/40 space-y-3 bg-surface/30">
                ...
                {row?.filePaths && row.filePaths.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {row.filePaths.slice(0, 3).map((fp) => (
                      <span key={fp} className="flex items-center gap-1 text-xs font-mono px-2 py-0.5 rounded border bg-surface" style={{ color: ACCENT, borderColor: `${withOpacity(ACCENT, OPACITY_20)}` }}>
                ...
                {row?.nextSteps && (
                  <div className="text-xs p-2 border-l-2 rounded font-medium"
                    style={{ backgroundColor: withOpacity(STATUS_WARNING, OPACITY_10), borderColor: STATUS_WARNING, color: STATUS_WARNING }}>
                    Next: {row.nextSteps}
                  </div>
                )}
              </div>
