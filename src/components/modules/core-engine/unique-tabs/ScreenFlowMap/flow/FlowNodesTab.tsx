                    ...
                    {(() => {
                      ...
                      return (
                        <>
                          ...
                          {row?.nextSteps && <p className="text-xs font-medium mt-2 p-2 border-l-2 rounded shadow-sm" style={{ backgroundColor: withOpacity(STATUS_WARNING, OPACITY_10), borderColor: STATUS_WARNING, color: STATUS_WARNING }}>Next: {row.nextSteps}</p>}
                        </>
                      );
                    })()}
                  </div>
            ...
            <div className="flex flex-col gap-2">
              {(Object.entries(INPUT_MODE_COLORS) as [InputMode, string][]).map(([mode, color]) => (
                <div key={mode} className="flex items-center gap-3 bg-surface p-2 rounded-lg border border-border/50">
                  <span className="px-2 py-0.5 rounded text-xs font-mono uppercase tracking-[0.15em] font-bold w-24 text-center shadow-sm" style={{ backgroundColor: `${withOpacity(color, OPACITY_12)}`, color, border: `1px solid ${withOpacity(color, OPACITY_25)}` }}>{mode}</span>
