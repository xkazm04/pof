        ...
        <BlueprintPanel color={ACCENT} className="p-4">
          ...
          <div className="bg-surface-deep/50 rounded-lg p-3 border border-border/30">
            ...
            <div className="w-full h-3 bg-surface rounded-full overflow-hidden border border-border/20 relative">
              ...
              />
              <div className="absolute top-0 bottom-0 left-[100%] w-0.5 z-20" style={{ backgroundColor: STATUS_ERROR, boxShadow: `0 0 4px ${STATUS_ERROR}` }} />
            </div>
            {lang.overflowWidgets.length > 0 && (
              <div className="mt-2.5 pt-3 border-t border-border/20">
                <span className="text-xs font-mono uppercase tracking-[0.15em] font-bold mb-2 flex items-center gap-1" style={{ color: STATUS_WARNING }}>
                  <AlertCircle className="w-3 h-3" /> Potential Overflow Areas
                </span>
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {lang.overflowWidgets.map(w => (
                    <span key={w} className="px-2 py-0.5 rounded border text-xs font-mono" style={{ backgroundColor: withOpacity(STATUS_WARNING, OPACITY_10), color: STATUS_WARNING, borderColor: withOpacity(STATUS_WARNING, OPACITY_30) }}>{w}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="flex items-center justify-center gap-8 w-full shrink-0">
            <div className="relative flex items-center justify-center w-32 h-32 rounded-full border-4 shadow-lg shrink-0"
              style={{
                ...
                backgroundColor: `${withOpacity(A11Y_OVERALL_SCORE > 80 ? STATUS_SUCCESS : STATUS_WARNING, OPACITY_8)}`,
              }}>
