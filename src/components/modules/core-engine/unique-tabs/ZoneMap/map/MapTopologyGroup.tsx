          ...
          <div className="bg-surface-deep p-4 rounded-xl border border-border/50 relative overflow-hidden">
            <div className="absolute top-0 right-0 p-3 opacity-20">
              {selectedZone.type === 'hub' ? <Anchor className="w-12 h-12" /> :
                selectedZone.type === 'boss' ? <MapIcon className="w-12 h-12" style={{ color: STATUS_ERROR }} /> :
            ...
            <div className="flex items-center gap-3 text-xs text-text-muted font-mono mb-2.5 relative z-10">
              <span className="bg-surface px-2 py-0.5 rounded border border-border/40" style={{ color: ACCENT_CYAN_LIGHT }}>
                LVL {selectedZone.levelRange}
              </span>
              <span className="px-2 py-0.5 rounded flex items-center gap-1 border"
                style={selectedZone.status === 'completed' ? {
                  ...
                  borderColor: withOpacity(STATUS_WARNING, OPACITY_30),
                } : {
