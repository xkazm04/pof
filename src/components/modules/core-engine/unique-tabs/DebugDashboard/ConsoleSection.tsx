  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
      ...
      <BlueprintPanel color={ACCENT} className="p-0 overflow-hidden">
              ...
              <motion.div key={entry.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="font-mono text-xs">
                ...
                {entry.status === 'pending' ? (
                  ...
                  <div className="pl-4 flex items-start gap-1.5">
                    ...
                    <span style={{ color: entry.status === 'success' ? `${STATUS_SUCCESS}cc` : `${STATUS_WARNING}cc` }}>
                      {entry.output}
                    </span>
                  </div>
                )}
        ...
        {/* Input */}
        <div className="border-t p-2 flex items-center gap-2" style={{ borderColor: `${ACCENT}18`, backgroundColor: `${ACCENT}06` }}>
