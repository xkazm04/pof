          ...
          <div className="flex flex-col items-center gap-3">
            <div className="relative">
              <svg width="110" height="110" viewBox="0 0 140 140">
                {MEMORY_SLICES.map((slice, i) => {
                  const r = 52;
                  const circ = 2 * Math.PI * r;
                  const offset = MEMORY_SLICES.slice(0, i).reduce((s, sl) => s + sl.pct, 0);
                  return (
                    <circle
                      key={slice.label}
                      cx="70" cy="70" r={r}
                      ...
                      style={{ filter: `drop-shadow(0 0 3px ${withOpacity(slice.color, OPACITY_25)})` }}
                    />
                  );
                })}
              </svg>
              <div className="absolute inset-0 flex items-center justify-center flex-col">
                <span className="text-lg font-mono font-bold" style={{ color: `${withOpacity(ACCENT, OPACITY_90)}`, textShadow: `0 0 12px ${withOpacity(ACCENT, OPACITY_25)}` }}>{MEMORY_TOTAL_MB}</span>
          ...
          <div className="space-y-3">
            ...
            <div className="border-t border-border pt-2 mt-2 space-y-1.5">
              <div className="flex justify-between text-xs font-mono uppercase tracking-[0.15em]">
                ...
                <span className="font-bold" style={{ color: `${withOpacity(ACCENT, OPACITY_90)}` }}>{MEMORY_PEAK_MB}MB</span>
              </div>
              <div className="flex justify-between text-xs font-mono uppercase tracking-[0.15em]">
