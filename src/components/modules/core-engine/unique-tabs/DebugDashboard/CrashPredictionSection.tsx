      ...
      <BlueprintPanel color={ACCENT} className="p-3">
        ...
        <div className="flex items-center gap-3 mb-2.5 p-3 rounded border" style={{ borderColor: `${ACCENT}18`, backgroundColor: `${ACCENT}06` }}>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-mono uppercase tracking-[0.15em] text-text-muted">OVERALL RISK</span>
          </div>
        ...
        {/* Risk factors */}
        <div className="text-[10px] font-mono uppercase tracking-[0.15em] text-text-muted mb-2">RISK FACTORS</div>
        <div className="space-y-1.5 mb-2.5">
          {CRASH_RISK_FACTORS.map((rf) => (
            <div key={rf.factor} className="flex items-center gap-3 px-2 py-1.5 rounded transition-colors border border-transparent hover:border-border hover:bg-surface-deep/30">
              ...
              <span className="text-[10px] font-mono uppercase tracking-[0.15em] w-36" style={{ color: `${ACCENT}cc` }}>{rf.factor}</span>
              ...
              <span className="text-[10px] font-mono text-text-muted ml-2 truncate">{rf.detail}</span>
            </div>
          ))}
        </div>

        {/* Recommendations */}
        <div className="text-[10px] font-mono uppercase tracking-[0.15em] text-text-muted mb-1.5">RECOMMENDED ACTIONS</div>
