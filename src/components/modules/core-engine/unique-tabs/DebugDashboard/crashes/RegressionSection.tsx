  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }}>
      ...
      <BlueprintPanel color={ACCENT} className="p-3">
        {/* Regression report */}
        <div className="text-xs font-mono uppercase tracking-[0.15em] text-text-muted mb-2">REGRESSION REPORT -- LATEST BUILD</div>
        <div className="space-y-1 mb-2.5">
          {REGRESSION_METRICS.map((rm) => (
            <div key={rm.metric} className="flex items-center gap-3 px-2 py-1.5 rounded hover:bg-surface-deep/30 transition-colors">
              ...
              <span className="text-xs font-mono uppercase tracking-[0.15em] w-28" style={{ color: `${withOpacity(ACCENT, OPACITY_80)}` }}>{rm.metric}</span>
              ...
              <span className="text-xs font-mono text-text-muted truncate flex-1">{rm.detail}</span>
              {rm.delta && (
        ...
        <div className="text-xs font-mono uppercase tracking-[0.15em] text-text-muted mb-2">HISTORICAL TREND ACROSS BUILDS</div>
        <div className="relative h-28 min-h-[200px] rounded-sm overflow-hidden mb-2" style={{ backgroundColor: `${withOpacity(ACCENT, OPACITY_5)}` }}>
          {[25, 50, 75].map((pct) => (
            <div key={pct} className="absolute left-0 right-0 border-t" style={{ top: `${100 - pct}%`, borderColor: `${withOpacity(ACCENT, OPACITY_8)}` }} />
          ))}
        ...
        {/* Bisect suggestion */}
        <div className="mt-3 p-2 rounded border flex items-start gap-1.5" style={{ borderColor: `${withOpacity(ACCENT, OPACITY_10)}`, backgroundColor: `${withOpacity(ACCENT, OPACITY_5)}` }}>
          <AlertTriangle className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" style={{ color: `${withOpacity(ACCENT, OPACITY_37)}` }} />
          <div>
            <div className="text-xs font-mono uppercase tracking-[0.15em] font-bold" style={{ color: `${withOpacity(ACCENT, OPACITY_80)}` }}>BISECT SUGGESTION</div>
            <div className="text-xs font-mono text-text-muted mt-0.5">
              ...
              <span className="mx-1" style={{ color: withOpacity(ACCENT, OPACITY_87) }}>git bisect</span>
