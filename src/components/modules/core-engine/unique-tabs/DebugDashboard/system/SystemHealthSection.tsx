...
import { STATUS_SUCCESS, STATUS_WARNING, STATUS_ERROR, OPACITY_5, OPACITY_8, OPACITY_12, OPACITY_25, OPACITY_30, OPACITY_50, withOpacity,
  OPACITY_80,
    ...
    <motion.div initial={prefersReduced ? { opacity: 1 } : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={motionSafe({ ...ANIMATION_PRESETS.entrance, delay: 0.15 }, prefersReduced)}>
      ...
      <BlueprintPanel color={ACCENT} className="p-3">
                ...
                <div className="flex-1 h-5 rounded-sm relative overflow-hidden" style={{ backgroundColor: withOpacity(ACCENT, OPACITY_5) }}>
                  <motion.div
                    ...
                    style={{
                      ...
                      boxShadow: `inset 0 0 10px ${withOpacity(bar.color, OPACITY_12)}`,
                    }}
                  />
                </div>
                <span className="text-xs font-mono font-bold w-14 text-right" style={{ color: bar.color, textShadow: `0 0 6px ${withOpacity(bar.color, OPACITY_25)}` }}>
                  {bar.ms.toFixed(1)}ms
                </span>
              </div>
            );
        ...
        <div className="mt-3 relative">
          ...
          <div className="h-6 rounded-sm flex overflow-hidden relative" style={{ backgroundColor: withOpacity(ACCENT, OPACITY_5) }}>
            {FRAME_TIME_BARS.map((bar) => (
              <motion.div
                key={bar.label}
                initial={{ width: 0 }}
                ...
                style={{
                  backgroundColor: withOpacity(bar.color, OPACITY_30),
                  borderRight: `1px solid ${withOpacity(ACCENT, OPACITY_8)}`,
                }}
                title={`${bar.label}: ${bar.ms}ms`}
              />
            ))}
            <div className="absolute right-0 top-0 bottom-0 w-[2px]" style={{ left: '100%', backgroundColor: withOpacity(ACCENT, OPACITY_50) }} />
          </div>
          <div className="flex justify-between mt-1">
            <span className="text-xs font-mono uppercase tracking-wider text-text-muted">0ms</span>
            <span className="text-xs font-mono uppercase tracking-wider" style={{ color: `${withOpacity(ACCENT, OPACITY_80)}` }}>{FRAME_TARGET_MS.toFixed(2)}ms (60fps)</span>
          </div>
        </div>
      </BlueprintPanel>
    </motion.div>
