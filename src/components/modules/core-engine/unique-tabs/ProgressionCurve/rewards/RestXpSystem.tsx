...
export function RestXpSystem() {
  return (
    <BlueprintPanel color={ACCENT_CYAN} className="p-5">
      <SectionHeader icon={Zap} label="Rest XP System" color={ACCENT_CYAN} />

      <div className="mt-2.5 space-y-4">
        <div>
          <div className="flex justify-between text-xs font-mono mb-1.5">
            <span className="text-text">Current Level Progress</span>
            <span style={{ color: ACCENT }}>{REST_XP_DATA.currentXP.toLocaleString()} / {REST_XP_DATA.nextLevelXP.toLocaleString()} XP</span>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <GlowStat label="Banked XP" value={REST_XP_DATA.bankedXP.toLocaleString()} color={ACCENT_CYAN} delay={0.1} />
          <GlowStat label="Multiplier" value={`${REST_XP_DATA.multiplier}x`} color={STATUS_WARNING} delay={0.2} />
          <GlowStat label="Kills to Deplete" value={`~${REST_XP_DATA.estimatedKills}`} color={ACCENT} delay={0.3} />
        </div>
