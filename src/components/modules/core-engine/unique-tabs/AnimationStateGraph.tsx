...
import { Activity, Play, AlertTriangle, AlertCircle, ChevronDown, RefreshCw, Cpu, Layers } from 'lucide-react';
...
import { STATUS_COLORS, TabHeader, PipelineFlow, SectionLabel, FeatureCard, LoadingSpinner, HeatmapGrid, LiveMetricGauge, TimelineStrip, SubTab, SubTabNavigation } from './_shared';
  ...
  return (
    <div className="space-y-2.5">
      ...
      <div className="mt-2.5 relative min-h-[300px]">
        <AnimatePresence mode="sync">
          {activeTab === 'state-graph' && (
            <motion.div key="state-graph" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="space-y-2.5">
      ...
      <SurfaceCard level={2} className="p-3 relative overflow-hidden group">
        ...
        <div className="flex gap-2.5 mb-3 relative z-10">
      ...
      <SurfaceCard level={2} className="p-3 relative overflow-hidden group">
        ...
        <div className="relative z-10 flex justify-center">
          <svg width={280} height={190} viewBox="0 0 280 190" className="overflow-visible">
            {/* Grid */}
            <rect x={35} y={8} width={228} height={158} fill="none" stroke="rgba(255,255,255,0.08)" strokeWidth="1" />
            {/* Grid lines - horizontal */}
            {[0.25, 0.5, 0.75].map((t) => {
              const yy = 166 - t * 158;
              return <line key={t} x1={35} y1={yy} x2={263} y2={yy} stroke="rgba(255,255,255,0.05)" strokeWidth="1" />;
            })}
            {/* Grid lines - vertical */}
            {[-90, 0, 90].map((deg) => {
            ...
            {/* Axis labels */}
            <text x={140} y={184} textAnchor="middle" className="text-[9px] font-mono fill-[var(--text-muted)]">Direction (deg)</text>
            <text x={10} y={87} textAnchor="middle" className="text-[9px] font-mono fill-[var(--text-muted)]" transform="rotate(-90 10 87)">Speed</text>
            {/* Tick labels - X axis */}
            {[-180, -90, 0, 90, 180].map((deg) => {
              const xx = 35 + ((deg + 180) / 360) * 228;
              return <text key={deg} x={xx} y={177} textAnchor="middle" className="text-[8px] font-mono fill-[var(--text-muted)]">{deg}</text>;
            })}
            {/* Tick labels - Y axis */}
            {[0, 0.5, 1].map((v) => {
              const yy = 166 - v * 158;
              return <text key={v} x={31} y={yy + 3} textAnchor="end" className="text-[8px] font-mono fill-[var(--text-muted)]">{v}</text>;
            })}
            {/* Animation clip dots */}
      ...
      <SurfaceCard level={2} className="p-3 relative overflow-hidden group">
        ...
        <div className="relative z-10 space-y-3">
          ...
          {activeTab === 'combos-montages' && (
            <motion.div key="combos-montages" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="space-y-2.5">
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-2.5">
                {/* Montage timeline */}
          ...
          <svg width={400} height={110} viewBox="0 0 400 110" className="overflow-visible">
            ...
            {COMBO_CHAIN_NODES.map((node) => (
              <g key={node.id}>
                ...
                <text x={node.x + 44} y={node.y - 9} textAnchor="middle" className="text-[10px] font-bold fill-[var(--text)]">{node.name}</text>
                <text x={node.x + 44} y={node.y + 2} textAnchor="middle" className="text-[8px] font-mono fill-[var(--text-muted)]">{node.montage}</text>
                <text x={node.x + 44} y={node.y + 13} textAnchor="middle" className="text-[9px] font-mono font-bold" fill={STATUS_ERROR}>
                  {node.damage} dmg
                </text>
              </g>
            ))}
      ...
      <SurfaceCard level={2} className="p-3 relative overflow-hidden group">
        ...
        <div className="relative z-10">
          ...
          {activeTab === 'retargeting' && (
            <motion.div key="retargeting" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="space-y-2.5">
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-2.5">
                <div className="space-y-2.5">
                    {/* Pipeline footer */}
      <SurfaceCard level={2} className="p-3 relative overflow-hidden group">
      ...
      <SurfaceCard level={2} className="p-3 relative overflow-hidden group">
        ...
        <div className="relative z-10 flex items-center gap-2.5">
          <svg width={120} height={120} viewBox="0 0 120 120" className="overflow-visible flex-shrink-0">
            ...
            <text x={60} y={117} textAnchor="middle" className="text-[7px] font-mono fill-[var(--text-muted)]">Origin</text>
            {/* Trajectories */}
          ...
          {activeTab === 'budget' && (
            <motion.div key="budget" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="space-y-2.5">
              {/* Asset list */}
