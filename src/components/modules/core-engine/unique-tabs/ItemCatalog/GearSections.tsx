...
export function EquipmentLoadoutSection() {
  return (
    <BlueprintPanel color={ACCENT} className="p-4">
      ...
      <p className="text-xs font-mono uppercase tracking-[0.15em] text-text-muted mb-3">Paper-doll layout with equipped items and stat contribution summary.</p>
          ...
          {(() => {
            ...
            return Object.entries(totals).map(([stat, d]) => (
              <div key={stat} className="space-y-0.5">
                ...
                <p className="text-xs text-text-muted opacity-60">{d.sources.join(', ')}</p>
              </div>
            ));
          })()}
        </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {set.pieces.map(piece => (
                  <div key={piece.slot} className="flex items-center gap-1.5 text-sm font-mono px-2 py-1 rounded border"
                    ...
                    {!piece.owned && <span className="text-red-400 text-xs">(missing)</span>}
                  </div>
                ))}
              </div>
                ...
                {set.bonuses.map(bonus => {
                  ...
                  return (
                    <div key={bonus.pieces} className="flex items-center gap-2 text-sm font-mono" style={{ opacity: active ? 1 : 0.4 }}>
                      ...
                      {active && <span className="text-emerald-400 text-xs font-mono uppercase tracking-[0.15em] ml-auto">ACTIVE</span>}
                    </div>
                  );
                })}
              </div>
