...
import { ACCENT, RARITY_COLORS, DUMMY_ITEMS, COMPARABLE_ITEMS, type ItemData } from './data';
...
export function CatalogGearTab({ moduleId, featureMap }: CatalogGearTabProps) {
  ...
  }, [items, categoryFilter, rarityFilter, searchQuery, sortBy]);
  ...
  useEffect(() => { setFocusedIndex(0); }, [currentPage, categoryFilter, rarityFilter, searchQuery, sortBy]);
        ...
        <BlueprintPanel color={ACCENT} className="p-3 sticky top-4 z-20 shadow-md space-y-3">
          <div className="flex flex-wrap items-center gap-3" role="toolbar" aria-label="Item filters">
            ...
            <select value={categoryFilter} onChange={e => { setCategoryFilter(e.target.value); setCurrentPage(0); }}
              ...
              <option value="Weapon">Weapon</option><option value="Armor">Armor</option><option value="Consumable">Consumable</option>
            </select>
          ...
          <motion.div ref={gridRef} layout role="grid" aria-label="Item catalog" onKeyDown={handleGridKeyDown}
            ...
            <AnimatePresence mode="popLayout">
              {pageItems.map((item, index) => (
                <TradingCard key={item.id} ref={(el: HTMLDivElement | null) => { cardRefs.current[index] = el; }}
