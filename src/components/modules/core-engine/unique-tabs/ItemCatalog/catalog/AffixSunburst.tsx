...
/* ── Arc path helper (module-scope to avoid recreation per render) ────── */
...
export function AffixSunburst({ tree, size }: { tree: ProbabilityEntry; size: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="overflow-visible">
      <circle cx={cx} cy={cy} r={innerR} fill={`${withOpacity(tree.color ?? ACCENT, OPACITY_12)}`} stroke={tree.color ?? ACCENT} strokeWidth="1.5" />
        ...
        return (
          ...
          >
            <path d={arc.d}
              ...
              style={{ transition: 'fill 0.15s, stroke-width 0.15s', filter: isHovered ? `drop-shadow(0 0 4px ${arc.fill})` : 'none' }}
            />
          </g>
        );
      })}
