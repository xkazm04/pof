...
import { ACCENT_CYAN, ACCENT_EMERALD, withOpacity, OPACITY_10, OPACITY_20, OPACITY_12 } from '@/lib/chart-colors';
      ...
      {ENEMY_ARCHETYPES.map((arch) => {
        ...
        return (
          <button key={arch.id} onClick={() => onSelectArchetype(arch.id)}
            ...
            style={{ borderColor: isActive ? color : 'var(--border)', backgroundColor: isActive ? `${withOpacity(color, OPACITY_10)}` : 'transparent', color: isActive ? color : 'var(--text-muted)' }}>
            <span className="w-4 h-4 rounded-sm flex items-center justify-center text-xs"
              style={{ backgroundColor: `${withOpacity(color, OPACITY_20)}`, color }}>{ARCHETYPE_ICONS[arch.id]}</span>
            {arch.name}
          </button>
        );
      })}
  ...
  return (
    <div className="flex items-center gap-3 text-xs font-mono uppercase tracking-[0.15em] text-text-muted/60 pt-1 border-t border-border/20 flex-wrap">
      <span className="flex items-center gap-1">
        <span className="w-3 h-3 rounded-sm border-2 cursor-grab" style={{ borderColor: ACCENT_EMERALD, backgroundColor: `${withOpacity(ACCENT_EMERALD, OPACITY_12)}` }} />
        drag to reposition
      </span>
      {selectedWave > 0 && (
        <span className="flex items-center gap-1">
