          ...
          {watchEntries.length > 0 && (
            <div className="space-y-1 max-h-48 overflow-y-auto custom-scrollbar">
              {watchEntries.map(([watchId, update]) => {
                const stamped = formatClock(update.timestamp);
                return (
                  <div key={watchId} className="flex items-center gap-2 px-2 py-1.5 rounded-lg border border-border/20 group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {/* Freshness: an unchanging value must not read as live. */}
                        <span className="text-2xs font-mono text-text-muted/70 ml-auto shrink-0">
                          {stamped ? `updated ${stamped}` : 'not yet stamped'}
                        </span>
                      </div>
                    </div>
                    <button
                      ...
                      className="p-0.5 rounded opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus-visible:opacity-100 transition-opacity flex items-center justify-center touch-target focus-ring"
