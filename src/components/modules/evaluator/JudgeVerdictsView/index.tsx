...
import { rollupVerdictsByCatalog, verdictTotals, type CatalogVerdictGroup } from './verdictRollup';
...
export function JudgeVerdictsView() {
  ...
  const [selected, setSelected] = useState<JudgeVerdict | null>(null);
    ...
    <div className="space-y-5" data-testid="judge-verdicts-view">
      {totals && (
        <div className="flex flex-wrap items-center gap-3 text-sm">
          ...
          <span className="text-text font-medium">{totals.total} verdicts</span>
          <span className="text-text-muted">·</span>
          <span style={{ color: qualityColor(totals.passCount / Math.max(1, totals.total) * 100) }}>{totals.passCount} pass</span>
          {totals.failCount > 0 && (
            <>
              <span className="text-text-muted">·</span>
...
function CatalogGroup({ group, onSelect }: { group: CatalogVerdictGroup; onSelect: (v: JudgeVerdict) => void }) {
  return (
        ...
        {group.verdicts.map((v) => (
          <li key={`${v.entityId}::${v.step}::${v.judge}`}>
            <button
              type="button"
              onClick={() => onSelect(v)}
              className="w-full flex items-center gap-3 px-3 py-2 text-left hover:bg-surface-hover transition-colors focus-ring"
            >
              ...
              <span className="flex-1 min-w-0">
                ...
                <span className="block text-2xs text-text-muted truncate font-mono">{v.entityId}</span>
              </span>
              <span className="text-2xs uppercase tracking-wider text-text-muted shrink-0">{JUDGE_LABEL[v.judge]}</span>
            </button>
          </li>
        ))}
      </ul>
