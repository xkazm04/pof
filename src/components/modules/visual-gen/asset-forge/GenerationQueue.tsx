...
import { Clock, CheckCircle, AlertCircle, Loader2, Trash2, X, Download, RotateCcw, ExternalLink } from 'lucide-react';
...
function JobCard({ job, now }: { job: GenerationJob; now: number }) {
  ...
  // is not necessarily reachable through the asset route (see `meshPreview`).
  const preview = outcome === 'complete' || outcome === 'rejected' ? meshPreview(job.meshPath) : null;
