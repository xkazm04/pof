...
// â”€â”€ Equipment slot positions for the character silhouette layout â”€â”€
...
export function InventoryGridDesigner({ onGenerate, isGenerating }: InventoryGridDesignerProps) {
  ...
  // â”€â”€ Grid dimension handlers â”€â”€
  ...
  // â”€â”€ Equipment slot toggles â”€â”€
  ...
  return (
    <div className="space-y-6 bg-[#03030a] p-6 rounded-2xl border border-violet-900/30 shadow-[inset_0_0_80px_rgba(167,139,250,0.05)] relative w-full overflow-hidden">
      ...
      {/* â”€â”€â”€ Grid Configuration â”€â”€â”€ */}
...
function SectionTab({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center justify-center gap-1.5 px-6 py-3 text-xs font-bold uppercase transition-all relative overflow-hidden group border-r border-violet-900/40 last:border-r-0 flex-1 hover:bg-white/5 ${active ? 'text-violet-200' : 'text-violet-500/50 hover:text-violet-300'
        }`}
    >
