...
import { CodeBlock } from '@/components/modules/core-engine/sub_ability/forge/CodeBlock';

/**
 * Audit-chip + collapsible composed-prompt panel. Turns the otherwise-invisible
 * prompt that was sent (or is about to be sent) to the LLM into an inspectable,
 * teachable surface so users can see at a glance which structural sections their
 * call actually carries — the audit chips are the one-glance signal, the body is
 * the exact composed prompt.
 *
 * Reused by the ability forge (post-run) and the daily checklist / quick-action
 * run path (pre-dispatch preview, via {@link TaskPromptInspector}).
 *
...
export interface PromptInspectorProps {
  /** The composed prompt to audit + display. `null` renders nothing. */
  prompt: string | null;
  /**
   * Optional summary of injected knowledge (known-asset domains, quality pack,
   * variant marker, …) surfaced as small tags in the expanded body — a plain
   * account of what got folded into the prompt beyond the structural sections.
   */
  knowledge?: string[];
  /** Start expanded (default collapsed — a low-friction disclosure). */
  defaultOpen?: boolean;
  /** Header label (default "Prompt Inspector"). */
  title?: string;
}

export function PromptInspector({ prompt, knowledge, defaultOpen = false, title = 'Prompt Inspector' }: PromptInspectorProps) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="font-mono uppercase tracking-[0.15em]">{title}</span>
          ...
          >
            <div className="border-t border-zinc-800 p-3 space-y-2">
              {knowledge && knowledge.length > 0 && (
                <div role="list" aria-label="Injected knowledge" className="flex flex-wrap items-center gap-1.5">
                  <span className="text-[11px] uppercase tracking-[0.15em] text-zinc-500 font-mono">Injected</span>
                  {knowledge.map((k) => (
                    <span
                      key={k}
                      role="listitem"
                      data-knowledge={k}
                      className="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-mono border border-zinc-700 text-zinc-300"
                    >
                      {k}
                    </span>
                  ))}
