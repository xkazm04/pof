...
describe('useLabPrefs', () => {
  ...
  it('persists + reflects an update', () => {
    const { result } = renderHook(() => useLabPrefs());
    act(() => result.current.setPrefs({ themeId: 'dark' }));
  ...
  it('adopts pre-populated storage after hydration', () => {
    localStorage.setItem('pof-lab-prefs', JSON.stringify({ themeId: 'dark', lastCatalogId: 'spellbook', lastEntityId: 'e1' }));
    const { result } = renderHook(() => useLabPrefs());
    expect(result.current.prefs.themeId).toBe('dark');
  ...
  it('adopts storage that differs only by lastEntityId', () => {
    localStorage.setItem('pof-lab-prefs', JSON.stringify({ themeId: 'light', lastEntityId: 'only-entity' }));
    const { result } = renderHook(() => useLabPrefs());
