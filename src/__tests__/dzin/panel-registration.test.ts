...
describe('pofRegistry CorePanel registration', () => {
  it('has arpg-combat-core registered', () => {
    expect(pofRegistry.has('arpg-combat-core')).toBe(true);
  });

  it('get returns definition with all required fields populated', () => {
    const def = pofRegistry.get('arpg-combat-core');
    expect(def).toBeDefined();
    expect(def!.type).toBe('arpg-combat-core');
  ...
  it('getByDomain arpg-combat includes core panel', () => {
    const panels = pofRegistry.getByDomain('arpg-combat');
    const types = panels.map((p) => p.type);
    expect(types).toContain('arpg-combat-core');
  });
