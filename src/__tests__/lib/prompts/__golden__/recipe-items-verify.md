...
| --- | --- | --- | --- | --- |
| Base Type & Rarity · baseType | UARPGInventoryComponent equips THIS item and activates the GameplayEffect bundle its base definition declares | the item is assigned to its declared equipment slot in UARPGInventoryComponent | UARPGAttributeSet (the stat targets THIS item modifies), UARPGItemDefinition (schema), DT_Items row "AshenBlade" | L2: UARPGItemDefinition compiles and DA_AshenBlade is seeded; L3: VSItemsDefinitionsTest — DA_AshenBlade loads and its base-type fields match THIS item’s declaration |
| Affixes · affixes | UARPGInventoryComponent::EquipItem creates one Infinite GameplayEffect handle per explicit affix THIS item rolls and one for its implicit, stores the handles on the slot, and removes them on unequip | THIS item is assigned to its equipment slot in UARPGInventoryComponent | UARPGAttributeSet (the target attribute for EACH affix THIS item declares), one GameplayEffect per affix THIS item declares (name each), one GameplayEffect for THIS item’s implicit | L2: UARPGItemDefinition and every declared affix or implicit GameplayEffect compile in Source/PoF/; L3: VSItemsDefinitionsTest — equipping Ashen Blade activates every declared handle and changes exactly the declared att… |
| Test Gate | UARPGInventoryComponent::EquipItem binds one Infinite GameplayEffect handle per affix THIS item declares and stores the handles by equipment slot | equip-slot assignment activates the handles; unequip removes every active handle created for THIS item | UARPGAttributeSet (the target attributes THIS item modifies), UARPGItemDefinition (DataAsset schema), DT_Items row "AshenBlade", the item-authoring seed script | L2: UARPGItemDefinition compiles and DA_AshenBlade is seeded; L3: VSItemsDefinitionsTest — Ashen Blade loads, equips on a test ability-system component, activates every declared GameplayEffect handle, and applies the de… |
| UE Packaging | UARPGItemDefinition DA_AshenBlade is represented by DT_Items row "AshenBlade"; UARPGInventoryComponent applies every GameplayEffect THIS item declares and binds its declared mesh to the appropriate socket | UARPGInventoryComponent loads DA_AshenBlade, assigns THIS item to its slot, and activates its GameplayEffect handles on the ability-system component | UARPGItemDefinition DA_AshenBlade, DT_Items row "AshenBlade", UARPGInventoryComponent (equip logic), UARPGAttributeSet (the target attributes THIS item modifies), the item-authoring seed script | L2: UARPGItemDefinition and every GameplayEffect THIS item declares compile in Source/PoF/ and DA_AshenBlade is seeded; L3: VSItemsDefinitionsTest — Ashen Blade loads, equips, activates its declared handles, and applies… |
